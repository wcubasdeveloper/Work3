...
exports.actualizarCarta = function(req, res, funcionName){
	...
	ejecutarQUERY_MYSQL(queryValidarNro, paramsValida, res, funcionName, function(res1, resultados1){
		...
		if(cantRegistros==0){
			...
			var queryUpdate = "Update CartaGarantia set nroCarta=?, fecha=?, idNosocomio=?, idServicioMedico=?, observaciones=?, monto = ?, idTipoAtencion=? where idCarta = ?";
		...
		if(cantRegistros==0){
			...
			ejecutarQUERY_MYSQL(queryInsertPrimeraProyeccion, paramsProy, res, funcionName, function(res, resultados){
				...
				if(idProyeccion>0){ // inserta la primera proyeccion
					// inserta la carta
					var queryInsert = "Insert into CartaGarantia (nroCarta, idEtapa, idTipoAtencion, estado, fecha, idNosocomio, idServicioMedico, nroCAT, codEvento, codAgraviado, observaciones, monto, primeraCarta, idPrimeraProyeccion)" +
						" values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
exports.getProyeccionPorAgraviado=function(req, res, funcionName){
	...
	var query = "Select a.codAgraviado, date_format(i.fechaHoraAccidente, '%d/%m/%Y %H:%i') as fechaAccidente, t.descripcion as descripcionAccidente, concat(pag.nombres,' ',pag.apellidoPaterno,' ',pag.apellidoMaterno) as nombreAgraviado, pag.nroDocumento as DNI, "+
		...
		" n.nombre as nombreNosocomio, co.nombre as nombreComisaria, a.diagnostico, a.diagnosticoAccidente as diagnosticoInicial, i.UIT from Agraviado a inner join Informe i on a.codEvento = i.codEvento "+
...
exports.getListaCartas = function(req, res, funcionName){
    var codEvento = req.query.codEvento;
	var codAgraviado = req.query.codAgraviado;
	var query = "Select c.idCarta, c.idEtapa as idCobertura, c.estado, concat(p.nombres,' ', p.apellidoPaterno,' ',p.apellidoMaterno) as nombreAgraviado, p.nroDocumento as DNI, "+
		" a.codAgraviado, a.codEvento, f.idFuneraria, f.nombre as nombreFuneraria, n.idNosocomio, n.nombre as nombreNosocomio, n.tipo as tipoNosocomio, n.idDistrito as distritoNosocomio, c.nroCarta, date_format(c.fecha, '%d/%m/%Y') as fecha, c.idTipoAtencion as tipoAsistencia, c.monto, c.idPrimeraProyeccion, "+
	...
	if(nroCarta!=""){ // verifica que el numero de carta no exista:
		...
		ejecutarQUERY_MYSQL(queryValidarNro, paramsValida, res, funcionName, function(res1, resultados1){
			...
			if(cantRegistros==0){ // el numero de carta esta disponible
				var query = "Update CartaGarantia set idEtapa=?, fecha=?, nroCarta=?, estado=?, idFuneraria=?, idNosocomio=?, esAmpliacion=?, idCartaAnterior=?, idTipoAtencion=?, idAuditor=?, idServicioMedico=?, observaciones=?, monto=? where idCarta=?";
				var parametros = [idEtapa, fechaCarta, nroCarta, estado, idFuneraria, idNosocomio, ampliacion, cartaPrevia, tipoAsistencia, auditor, servicioMedico, observaciones, monto, idCarta];
		...
		ejecutarQUERY_MYSQL(queryValidarNro, paramsValida, res, funcionName, function(res1, resultados1){
			...
			if(cantRegistros==0){ // el numero de carta esta disponible
                ...
                ejecutarQUERY_MYSQL(queryUpdateAgraviado, params, res, funcionName, function(res, resultados){
                    ...
                    var parametros = [idEtapa, fechaCarta, nroCarta, tipoAsistencia, idFuneraria, idNosocomio, ampliacion, servicioMedico, nroCAT, codEvento, codAgraviado, observaciones, monto, cartaPrevia, auditor];
...
exports.getTotalFacturaPorEtapa = function(req, res, funcionName){
	var codAgraviado = req.query.codAgraviado
	var query = "select (select sum(f1.monto) from Gasto f1 where f1.codAgraviado=ag.codAgraviado and f1.idEtapa='1') as monto1, "+
	"(select sum(f2.monto) from Gasto f2 where f2.codAgraviado=ag.codAgraviado and f2.idEtapa='5') as monto5 "+
...
exports.getTotalOrdenesPorEtapa = function(req, res, funcionName){
	var codAgraviado = req.query.codAgraviado
	var query = "select (select sum(o1.monto) from OrdenPago o1 where o1.codAgraviado=ag.codAgraviado and o1.idEtapa='1' and o1.estado in ( 'B', 'P' )) as monto1, "+
	"(select sum(o2.monto) from OrdenPago o2 where o2.codAgraviado=ag.codAgraviado and o2.idEtapa='5' and o2.estado in ( 'B', 'P' )) as monto5 "+
	" from Agraviado ag where ag.codAgraviado=?";
	var params = [codAgraviado];
	ejecutarQUERY_MYSQL(query, params, res, funcionName);
}
