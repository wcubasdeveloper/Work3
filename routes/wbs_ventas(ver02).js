...
exports.guardarGuia = function(req, res, funcionName){
	...
	if(tipo=='DEV' || tipo=='DIST'){
		...
		var queryInsertGuia = "Insert into Guia_movimiento_cabecera(tipoOperacion, fechaOperacion, idConcesionario, idUsuario, idUsuarioResp, nroGuiaManual) values (?,?,?,?,?,?)";
...
exports.getPromotores = function(req, res, funcionName){
	...
	var query = "Select p.idPromotor, concat(u.Nombres,' ',u.Apellidos) as nombreUsuario from Promotor p inner join UsuarioIntranet u on p.idUsuario = u.idUsuario "+queryWhere+" order by u.Nombres";
	ejecutarQUERY_MYSQL(query, [], res, funcionName);	
}
...
exports.generarReporteCertificado = function(req, res, funcionName){
	...
	switch(tipoReporte){
		...
		case 'PRO':
			...
			var query = "select c.nroCertificado, date_format(c.fechaOperacion, '%d/%m/%Y') as fechaOperacion , if(c.estado='V', 'VEND', g.tipoOperacion) as tipOperacion, c.estado, c.idUbicacion, if(co.tipoPersona='J', co.razonSocial, concat(co.nombres,' ',co.apellidoPaterno,' ',co.apellidoPaterno)) as nombreConcesionario, LPAD(c.idGuia, 4, '0') as idGuia, "+
				" concat(u.Nombres,' ', u.Apellidos) as nombreUsuario, g.nroGuiaManual "+
				" from Certificado c "+			
				" inner join Concesionario con on c.idUbicacion = con.idConcesionario "+
				" left join Persona co on con.idPersona = co.idPersona "+
				" inner join Guia_movimiento_cabecera g on c.idGuia = g.idGuia_movimiento_cabecera "+
				" inner join UsuarioIntranet u on g.idUsuario = u.idUsuario "+
				" where c.tipOperacion = 'E' and con.idPromotor=? and con.diaSemanaAtt in ("+dias+") and c.idUbicacion>0 order by c.nroCertificado";
