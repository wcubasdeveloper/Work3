...
exports.buscarPlaca = function (req, res, funcionName) {
    ...
    var query = "Select v.idCategoria, v.idVehiculo, ucv.idUso, v.marca, v.modelo, v.anno, v.nroAsientos, v.nroSerieMotor, v.idUsoClaseVehiculo as idClase from Vehiculo v inner join UsoClaseVehiculo ucv on v.idUsoClaseVehiculo = ucv.idUsoClaseVehiculo where v.placa = ? ";
        ...
        function (idPersona_Asociado) {
            ...
            ejecutarQUERY_MYSQL(query, parametros, res, funcionName,
                function (res, resultados) {
                    ...
                    abstractGuardarActualizarVehiculo(res, funcionName, vehiculo,
                        function (idVehiculo_CAT) {
                            ...
                            var queryInsertCat = "Insert into Cat (nroCAT, idAsociado, placa, marca, modelo, annoFabricacion, nMotorserie, fechaInicio, " +
                                "fechaCaducidad, idConcesionario, fechaEmision, fechaControlInicio, fechaControlFin, idVehiculo, conDeuda, fechaLiquidacion, " +
                                "prima, comision, aporte, tipoPersona, idPromotor) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                            ...
                            req.query.fechaCP_inicio, req.query.fechaCP_fin, idVehiculo_CAT, req.query.conDeuda, req.query.fechaLiquidacion, req.query.prima,
...
function abstractGuardarActualizarVehiculo(res, funcionName, vehiculo, callback) { // Guarda o actualiza un vehiculo
    var queryInsert = "Insert into Vehiculo (placa, idCategoria,idUsoClaseVehiculo, marca, modelo, anno, nroSerieMotor, nroAsientos) values (?,?,?,?,?,?,?,?)";
    var queryUpdate = "Update Vehiculo set idCategoria=?,idUsoClaseVehiculo=?, marca=?, modelo=?, anno=?, nroSerieMotor=?, nroAsientos=? where idVehiculo=?";
    if (vehiculo.idVehiculo == 0) { // Vehiculo  no existe, por lo tanto se registra uno nuevo
        ejecutarQUERY_MYSQL(queryInsert, [vehiculo.placa, vehiculo.idCategoria, vehiculo.idClase, vehiculo.marca, vehiculo.modelo, vehiculo.anno, vehiculo.nroSerieMotor, vehiculo.nroAsientos], res, funcionName, function (res, resultados) {
        ...
        ejecutarQUERY_MYSQL(queryUpdate, [vehiculo.idCategoria, vehiculo.idClase, vehiculo.marca, vehiculo.modelo, vehiculo.anno, vehiculo.nroSerieMotor, vehiculo.nroAsientos, vehiculo.idVehiculo], res, funcionName, function (res, resultados) {
            if (typeof callback == 'function') {
