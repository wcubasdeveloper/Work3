...
exports.consultarInfoPromotor = function (req, res, funcionName) {
	...
	var query = "Select p.idPromotor, u.idUsuario, u.Nombres, u.Apellidos, u.UName, u.password, u.DNI, u.telefono, u.email, u.idPerfil1, u.idPerfil2, u.idPerfil3, u.idLocal, " +
		" u.idArea, p.nroConcesionarios, if(p.ultVisitaConcesionario=0, '',p.ultVisitaConcesionario) as ultVisitaConcesionario, date_format(p.ultVisitaFecha, '%d/%m/%Y') as ultVisitaFecha, idConcesionarioPromotor from Promotor p inner join UsuarioIntranet u on p.idUsuario = u.idUsuario where p.idPromotor = ? ";
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName);
}
...
exports.guardarPromotor = function (req, res, funcionName) {
	...
	var idLocal = req.query.idLocal;
	var idUsuario = req.query.idUsuario;
	var idConcesionarioPromotor = req.query.idConcesionarioPromotor;
	var arrayParametros = [idNombres, idApellidos, idDNI, idUName, idClave, idPerfil1, idPerfil2, idPerfil3, idTelefono, idCorreo, idArea, idLocal];
	var query = "";
	if (idUsuario == "0") { // registra un usuario y luego el procurador
		query = "Insert into UsuarioIntranet (Nombres, Apellidos, DNI, UName, password, idPerfil1, idPerfil2, " +
	...
	ejecutarQUERY_MYSQL_Extra(idUsuario, query, arrayParametros, res, funcionName, function (res, resultados, idUsuario) {
		if (idUsuario == "0") {
			var idUsuario = resultados.insertId;
			var queryInsert = "Insert into Promotor (idUsuario,idConcesionarioPromotor) values (?,?)";
			ejecutarQUERY_MYSQL(queryInsert, [idUsuario, idConcesionarioPromotor], res, funcionName, "insertId");
		} else {
			...
			var queryUpdatePromotor = "Update Promotor set nroConcesionarios=?, ultVisitaConcesionario=?, " +
				" ultVisitaFecha=?, idConcesionarioPromotor=? where idUsuario = ?";
			var arrayParametros = [nroConcesionarios, ultimoConcesionario, ultimaVisita, idConcesionarioPromotor, idUsuario];
	...
	ejecutarQUERY_MYSQL_Extra(idPersona, query, arrayParametros, res, funcionName, function (res, resultados, idPersona) {
		...
		if (idResponsable == "0" && req.query.dni_resp != "" && req.query.nombres_resp != "") { // registra el responsable
			...
			if (idPersona == "0") {
				...
				var queryInsert = "Insert into Concesionario (idPersona, idSede, idPromotor, diaSemanaAtt, estado, fechaAfiliacion, idResponsable) values (?,?,?,?,?,"+ (afiliacion ? afiliacion : "NULL")+",?)";
				var local = req.query.local;
				var promotor = req.query.promotor;
				var visita = req.query.visita;
				var estado = req.query.estado;
				//var parametros = [idPersona, local, promotor, visita, estado, afiliacion, idResponsable];
				var parametros = [idPersona, local, promotor, visita, estado, idResponsable];
...
exports.getListaEmpresaTransp = function (req, res, funcionName) {
	var nombreEmpresa = req.query.nombreEmpresa

	var queryWhere = new QueryWhere("");
	if (nombreEmpresa != "") {
		queryWhere.validarWhere(" ( p.razonSocial like '%" + nombreEmpresa + "%' ) ");
	}

	var page = req.query.page;
	var cantPaginas = req.query.cantPaginas;
	var registrosxpagina = req.query.registrosxpagina;
	var query = "select e.idEmpresaTransp, p.razonSocial, e.nombreCorto, e.nroResolucion, p.telefonoFijo, " +
		" if(pl.tipoPersona='J', pl.razonSocial, concat(pl.nombres,' ',pl.apellidoPaterno,' ',pl.apellidoMaterno)) as " +
		" representanteLegal from EmpresaTransp e " +
		" inner join Persona p on e.idPersona = p.idPersona " +
		" left join Persona pl on e.idRepLegal = pl.idPersona " + queryWhere.getQueryWhere() + " order by e.idEmpresaTransp desc "

	query = agregarLimit(page, registrosxpagina, query);
	ejecutarQUERY_MYSQL(query, [], res, funcionName, function (res, resultados) {
		if (resultados.length > 0) {
			if (cantPaginas == 0) {
				var queryCantidad = "Select count(*) as cantidad from EmpresaTransp pr inner join Persona p on pr.idPersona = p.idPersona " + queryWhere.getQueryWhere();
				ejecutarQUERY_MYSQL_Extra(resultados, queryCantidad, [], res, funcionName, function (res, rows, resultados) {
					var cantidadPag = Math.ceil(rows[0].cantidad / registrosxpagina);
					resultados[0].numeroPaginas = cantidadPag;
					enviarResponse(res, resultados);
...
exports.getListaProveedor = function (req, res, funcionName) {
	var nombreProveedor = req.query.nombreProveedor

	var queryWhere = new QueryWhere("");
	if (nombreProveedor != "") {
		queryWhere.validarWhere(" ( p.razonSocial like '%" + nombreProveedor + "%' or concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) like '%" + nombreProveedor + "%' ) ");
	}
	...
	var query = "SELECT pr.idProveedor, if(p.tipoPersona='N', concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno), p.razonSocial) as nombreProveedor, " +
		...
		" FROM Proveedor pr " +
		" inner join Persona p on pr.idPersona = p.idPersona " + queryWhere.getQueryWhere() + " order by pr.idProveedor desc /*if(p.tipoPersona='N', concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno), p.razonSocial)*/";

	query = agregarLimit(page, registrosxpagina, query);
	ejecutarQUERY_MYSQL(query, [], res, funcionName, function (res, resultados) {
		if (resultados.length > 0) {
			if (cantPaginas == 0) {
				var queryCantidad = "Select count(*) as cantidad from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona " + queryWhere.getQueryWhere();
				ejecutarQUERY_MYSQL_Extra(resultados, queryCantidad, [], res, funcionName, function (res, rows, resultados) {
					var cantidadPag = Math.ceil(rows[0].cantidad / registrosxpagina);
					resultados[0].numeroPaginas = cantidadPag;
					enviarResponse(res, resultados);
...
exports.getListaVehiculo = function (req, res, funcionName) {
	var placa = req.query.placa

	var queryWhere = new QueryWhere("");
	if (placa != "") {
		queryWhere.validarWhere(" ( v.placa like '%" + placa + "%' ) ");
	}
	...
	var cantPaginas = req.query.cantPaginas;
	var registrosxpagina = req.query.registrosxpagina;
	var query = "select v.idVehiculo,v.placa,v.idCategoria,vc.nombreCategoria,u.nombreUso,c.nombreClase,v.nroSerieMotor, " +
		"v.marca,v.modelo,v.anno,v.nroAsientos " +
		"from Vehiculo v " +
		"left join Vehiculo_Categoria vc on vc.idCategoria=v.idCategoria " +
		"inner join UsoClaseVehiculo ucv on ucv.idUsoClaseVehiculo=v.idUsoClaseVehiculo " +
		"inner join Uso_Vehiculo u on u.idUso=ucv.idUso " +
		"inner join Clase_Vehiculo c on c.idClase=ucv.idClaseVehiculo " + queryWhere.getQueryWhere() +
...
//LIsta de Informes Cerrados
exports.getListaInformec = function (req, res, funcionName) {
	var codEvento = req.query.codEvento
	var queryWhere = new QueryWhere("");
	if (codEvento != "") {
		queryWhere.validarWhere(" ( i.codEvento like '%" + codEvento + "%' ) ");
	}
	...
	var cantPaginas = req.query.cantPaginas;
	var registrosxpagina = req.query.registrosxpagina;
	var query = "select i.idInforme,i.codEvento,i.nroCAT,c.placa, date_format(i.fechaInforme, '%d/%m/%Y') as fechaInforme, " +
		"i.UIT,i.sueldoMinVital " +
		"from Informe i " +
		"inner join Cat c on c.nroCAT=i.nroCAT " + queryWhere.getQueryWhere() +
		" order by i.codEvento"
	query = agregarLimit(page, registrosxpagina, query);
	ejecutarQUERY_MYSQL(query, [], res, funcionName, function (res, resultados) {
		if (resultados.length > 0) {
			if (cantPaginas == 0) {
...
exports.consultarInfoEmpresaTransp = function (req, res, funcionName) {
	var id = req.query.id;
	var arrayParametros = [id];
	var query = "select e.idEmpresaTransp, p.idPersona , p.razonSocial, p.nroDocumento as ruc, e.nombreCorto, e.nroResolucion, p.telefonoFijo, " +
		" p.idDistrito, p.calle, pl.idPersona as idRepresentanteLegal, pl.tipoPersona, pl.nombres, pl.apellidoPaterno, pl.apellidoMaterno, pl.razonSocial as razonSocialRepresentante, " +
		" pl.nroDocumento from EmpresaTransp e " +
		" inner join Persona p on e.idPersona = p.idPersona " +
		" left join Persona pl on e.idRepLegal = pl.idPersona where e.idEmpresaTransp = ?"
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName);
}
function verificarExistenciaNosocomioFuneraria(idProveedor, nombre, tipoProveedor, res, funcionName, callback) {

	// reinicia los NOSOCOMIOS Y FUNERARIAS QUE CONTENGAN EL IDPROVEEDOR:
	var queryReiniciarNosocomio = "Update Nosocomio set idProveedor = null where idProveedor = ?"
	var params = [idProveedor]

	ejecutarQUERY_MYSQL(queryReiniciarNosocomio, params, res, funcionName, function () {
		var queryReiniciarFuneraria = "Update Funeraria set idProveedor = null where idProveedor = ?"
		var params = [idProveedor]
		ejecutarQUERY_MYSQL(queryReiniciarFuneraria, params, res, funcionName, function () {
			...
			var query = "Select " + pk_id + " as id from " + tabla + " where nombre = ?"
			var parametros = [nombre]
			ejecutarQUERY_MYSQL(query, parametros, res, funcionName, function (res, results) {
				callback(results)
			})
...
function guardarNosocomioFuneraria(proveedorId, req, res, funcionName) {
	...
	switch (tipoPersona) {
		case 'N':
			nombreProveedor = req.query.nombres + " " + req.query.apellidoPaterno + " " + req.query.apellidoMaterno
			break
		case 'J':
			nombreProveedor = req.query.razonSocial
			break
	}
	console.log("verifica si existen nosocomios o funerarias relacionadas al proveedor ...")
	verificarExistenciaNosocomioFuneraria(proveedorId, nombreProveedor, tipoProveedor, res, funcionName, function (nosocomiosFunerarias) {
		if (nosocomiosFunerarias.length == 0) {
			console.log("No existe ningun nosocomio / funeraria con el mismo nombre del proveedor ...")
			...
			// actualiza el idProveedor en el o los nosocomios existentes

			var ids = []
			for (var i = 0; i < nosocomiosFunerarias.length; i++) {
				ids.push(nosocomiosFunerarias[i].id.toString())
			}

			console.log("Existen nosocomios / funerarias con el mismo nombre del proveedor ... ids = " + ids)

			var queryUpdate = ""
			var parametros = [proveedorId]
			var tipoProveedor = req.query.tipoProveedor;
			switch (tipoProveedor) {
				case 'N':
					queryUpdate = "Update Nosocomio set idProveedor = ? where idNosocomio in (" + ids + ")"
					break;
				case 'F':
					queryUpdate = "Update Funeraria set idProveedor = ? where idFuneraria in (" + ids + ")"
					break;
			}

			ejecutarQUERY_MYSQL(queryUpdate, parametros, res, funcionName, "false")
		}
	})
}

function guardarRepresentanteLegal(req, res, funcionName, callback) {

	var idPersona = req.query.idRepresentante
	var tipoPersona = req.query.tipoPersona
	var nombres = req.query.nombres
	var apellidoPaterno = req.query.apellidoPaterno
	var apellidoMaterno = req.query.apellidoMaterno
	...
	var arrayParametros = [tipoPersona, nombres, apellidoPaterno, apellidoMaterno, razonSocial, nroDocumento];
	var query = "";
	if (idPersona == "0") { // registra un usuario y luego el proveedor
		query = "Insert into Persona (tipoPersona, nombres, apellidoPaterno, apellidoMaterno, razonSocial, nroDocumento)" +
			" values (?,?,?,?,?,?)";
	} else { // actualiza el registro
		arrayParametros.push(idPersona);
		query = "Update Persona set tipoPersona=?, nombres=?, apellidoPaterno=?, apellidoMaterno=?, razonSocial=?, nroDocumento=? where idPersona=?";
	}
	ejecutarQUERY_MYSQL_Extra(idPersona, query, arrayParametros, res, funcionName, function (res, resultados, idPersona) {
		if (idPersona == "0") {
			var idPersona = resultados.insertId;
...
exports.guardarEmpresaTransp = function (req, res, funcionName) {

	var idPersona = req.query.idPersona;
	var razonsocialEmpresa = req.query.razonsocialEmpresa
	var ruc = req.query.ruc
	var telefonoFijo = req.query.telefonoFijo
	var calle = req.query.calle
	var distrito = req.query.distrito

	var arrayParametros = [razonsocialEmpresa, ruc, distrito, calle, telefonoFijo];
	var query = "";

	if (idPersona == "0") {
		query = "Insert into Persona (tipoPersona, razonSocial, nroDocumento, idDistrito, calle, telefonoFijo)" +
			" values ('J',?,?,?,?,?)";
	} else {
		arrayParametros.push(idPersona);
		query = "Update Persona set tipoPersona='J', razonSocial=?, nroDocumento=?, idDistrito=?, calle=?, telefonoFijo=? where idPersona = ?"
	}
	ejecutarQUERY_MYSQL_Extra(idPersona, query, arrayParametros, res, funcionName, function (res, resultados, idPersona) {
		if (idPersona == "0") {
			guardarRepresentanteLegal(req, res, "guardarRepresentanteLegal", function (idRepresentanteLegal) {
				var idPersona = resultados.insertId;
				var queryInsert = "Insert into EmpresaTransp(idPersona, idRepLegal, nroResolucion, fechaRegistro, nombreCorto) values (?,?,?,now(),?)"
				var parametros = [idPersona, idRepresentanteLegal, req.query.nroResolucion, req.query.nombreCorto];
				ejecutarQUERY_MYSQL(queryInsert, parametros, res, funcionName, "insertId")
			})
		} else {
			guardarRepresentanteLegal(req, res, "guardarRepresentanteLegal", function (idRepresentanteLegal) {
				var arrayParametros = [req.query.nroResolucion, req.query.nombreCorto, req.query.idEmpresaTransp];
				var queryUpdate = "Update EmpresaTransp set nroResolucion=?, nombreCorto= ? where idEmpresaTransp = ?"
...
exports.guardarProveedor = function (req, res, funcionName) {
	...
	ejecutarQUERY_MYSQL_Extra(idPersona, query, arrayParametros, res, funcionName, function (res, resultados, idPersona) {
		if (idPersona == "0") {
            ...
            /*if(req.query.tipoProveedor=='N' || req.query.tipoProveedor=='F'){
                var idProveedor = req.query.idProveedor;
                guardarNosocomioFuneraria(idProveedor, req, res, "guardarNosocomioFuneraria")
            }}*/
		}
	});
}
exports.getListaAsociado = function (req, res, funcionName) {
	var nombreAsociado = req.query.nombreAsociado

	var queryWhere = new QueryWhere("");
	if (nombreAsociado != "") {
		queryWhere.validarWhere(" ( p.razonSocial like '%" + nombreAsociado + "%' or concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) like '%" + nombreAsociado + "%' ) ");
	}
	//** Parametros de Paginacion ****
	var page = req.query.page;
	var cantPaginas = req.query.cantPaginas;
	var registrosxpagina = req.query.registrosxpagina;
    /*
    var query = "SELECT pr.idProveedor, if(p.tipoPersona='N', concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno), " +
        ...
        " END as tipoProveedor "+
        " FROM Proveedor pr "+
        " inner join Persona p on pr.idPersona = p.idPersona "+queryWhere.getQueryWhere()+
        " order by pr.idProveedor desc"; //if(p.tipoPersona='N', concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno), p.razonSocial)
    */
	var query = "select a.idAsociado, if(p.tipoPersona='N', concat(COALESCE(nombres,''),' ',COALESCE(apellidoPaterno,''),' ', " +
		" COALESCE(apellidoMaterno,'')), p.razonSocial) as nombre, " +
		" p.nroDocumento, p.calle, p.telefonoMovil, p.email  " +
		" from Asociado a " +
	...
	ejecutarQUERY_MYSQL(query, [], res, funcionName, function (res, resultados) {
		if (resultados.length > 0) {
			if (cantPaginas == 0) {
				var queryCantidad = "Select count(*) as cantidad from Asociado a inner join Persona p on a.idPersona = p.idPersona " + queryWhere.getQueryWhere();
				ejecutarQUERY_MYSQL_Extra(resultados, queryCantidad, [], res, funcionName, function (res, rows, resultados) {
					var cantidadPag = Math.ceil(rows[0].cantidad / registrosxpagina);
					resultados[0].numeroPaginas = cantidadPag;
					enviarResponse(res, resultados);
...
exports.consultarInfoVehiculo = function (req, res, funcionName) {
	var id = req.query.id;
	var arrayParametros = [id];
	var query = "Select v.idVehiculo,v.idUsoClaseVehiculo,ucv.idUso, ucv.idClaseVehiculo, " +
		"v.idCategoria, v.placa,v.nroSerieMotor,v.marca,v.modelo,v.anno,v.nroAsientos " +
		" from Vehiculo v " +
		" inner join UsoClaseVehiculo ucv on ucv.idUsoClaseVehiculo = v.idUsoClaseVehiculo " +
		" where idVehiculo = ? ";
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName);
}
exports.actualizarVehiculo = function (req, res, funcionName) {
	var placa = req.query.placa;
	var idCategoria = req.query.idCategoria;
	var idUsoClase = req.query.idUsoClase;
	var marca = req.query.marca;
	var modelo = req.query.modelo;
	var anno = req.query.anno;
	var serieMotor = req.query.serieMotor;
	var nroAsientos = req.query.nroAsientos;
	var idVehiculo = req.query.idVehiculo;
	var arrayParametros = [placa, idCategoria, idUsoClase, marca, modelo, anno, serieMotor, nroAsientos, idVehiculo];
	var query = "Update Vehiculo set placa=?, idCategoria=?, idUsoClaseVehiculo=?, marca=?, modelo=?, anno=?, " +
		" nroSerieMotor=?, nroAsientos=? where idVehiculo=?";
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, function (res, results) {
		//Despues debe actualizar los CATS que usen este vehiculo
		var queryUpdateCat = "Update Cat set placa=?, marca=?, modelo=?, annoFabricacion=?, nMotorserie=? " +
			" where idVehiculo=? ";
		var arrayParametros1 = [placa, marca, modelo, anno, serieMotor, idVehiculo];
		ejecutarQUERY_MYSQL(queryUpdateCat, arrayParametros1, res, funcionName, "affectedRows");
	});
}
//Informe Cerrado
exports.consultarInfoInformec = function (req, res, funcionName) {
	var id = req.query.id;
	var arrayParametros = [id];
	var query = "Select codEvento,UIT,sueldoMinVital " +
		" from Informe " +
		" where idInforme = ? ";
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName);
}
exports.actualizarInformec = function (req, res, funcionName) {
	var UIT = req.query.UIT;
	var sueldoMinVital = req.query.sueldoMinVital;
	var idInforme = req.query.idInforme;
	var arrayParametros = [UIT, sueldoMinVital, idInforme];
	var query = "Update Informe set UIT=?, sueldoMinVital=? where idInforme=?";
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, "affectedRows");
}
...
exports.getVariablesGlobales = function (req, res, funcionName) {
	var query = "Select UIT, sueldoMinVital, plantillaCuerpoNotificacion, plantillaPieNotificacion, plantillaCuerpoOPA, plantillaPieOPA, FACTNroSerie, BVNroSerie, NCNroSerie, NCCorrelativoActual, FACTCorrelativoActual, BVCorrelativoActual, NroAutSUNAT , NroMaqRegistradora  from ConstantesGenerales";
	ejecutarQUERY_MYSQL(query, [], res, funcionName)
}
exports.actualizarVariablesGlobales = function (req, res, funcionName) {
	...
	var plantillaCuerpoOPA = req.body.plantillaCuerpoOPA;
	var plantillaPieOPA = req.body.plantillaPieOPA;
	...
	var FACTNroSerie = req.body.FACTNroSerie
	var BVNroSerie = req.body.BVNroSerie
	var NCNroSerie = req.body.NCNroSerie
	var NCCorrelativoActual = req.body.NCCorrelativoActual
	var FACTCorrelativoActual = req.body.FACTCorrelativoActual
	var BVCorrelativoActual = req.body.BVCorrelativoActual
	var NroAutSUNAT = req.body.NroAutSUNAT
	var NroMaqRegistradora = req.body.NroMaqRegistradora

	var arrayParametros = [plantillaCuerpo, plantillaPie, sueldoMinVital, UIT, plantillaCuerpoOPA, plantillaPieOPA, FACTNroSerie, BVNroSerie, NCNroSerie, NCCorrelativoActual, FACTCorrelativoActual, BVCorrelativoActual, NroAutSUNAT, NroMaqRegistradora]
	var query = "Update ConstantesGenerales set plantillaCuerpoNotificacion=?, plantillaPieNotificacion=?, sueldoMinVital=?, UIT = ?, plantillaCuerpoOPA=?, plantillaPieOPA=?, " +
		"FACTNroSerie=?, BVNroSerie=?, NCNroSerie=?, NCCorrelativoActual=?, FACTCorrelativoActual=?, BVCorrelativoActual=?, NroAutSUNAT=?, NroMaqRegistradora=?";
	if (fechaUIT != "") {
...
exports.validarNombreNosocomio = function (req, res, funcionName) {
	var nombreNosocomio = req.query.nombreNosocomio
	var idNosocomio = req.query.idNosocomio

	var query = "Select idNosocomio from Nosocomio where nombre = ? and idNosocomio != ?"
	var params = [nombreNosocomio, idNosocomio]
	ejecutarQUERY_MYSQL(query, params, res, funcionName)
}
function obtenerNosocomioProveedor(razonSocial, req, res, funcionName, callback) { // retorna el idProveedor insertado o buscado

	var query = "Select pr.idProveedor from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona where p.tipoPersona = 'J' and p.razonSocial = ?"

	var parametros = [razonSocial]

	ejecutarQUERY_MYSQL(query, parametros, res, funcionName, function (res, results) {
		if (results.length == 0) { // inserta el proveedor

			var tipoPersona = 'J';
			var distrito = req.query.distrito;
			var calle = req.query.direccion;

			var arrayParametros = [tipoPersona, razonSocial, distrito, calle];

			var query = "Insert into Persona (tipoPersona, razonSocial, idDistrito, calle) values (?,?,?,?)"

			ejecutarQUERY_MYSQL(query, arrayParametros, res, "InsertaProveedorPersona", function (res, results) {
				var idPersona = results.insertId
				console.log("persona registrada : " + idPersona)
				console.log("registra proveedor ...")

				var queryInsertProveedor = "Insert into Proveedor(idPersona, tipoProveedor) values (?, 'N')"
				var params = [idPersona]

				ejecutarQUERY_MYSQL(queryInsertProveedor, params, res, "InsertaProveedor", function (res, resultados) {
					var idProveedor = resultados.insertId
					console.log("proveedor registrado : " + idProveedor)
			...
			var idProveedor = results[0].idProveedor
			console.log("proveedor encontrado : " + idProveedor)
...
exports.guardarNosocomio = function (req, res, funcionName) {
	...
	if (idNosocomio == "0") { // Registro
		...
		obtenerNosocomioProveedor(nombre, req, res, "obtenerNosocomioProveedor", function (idProveedor) {
			var idNosocomio = req.query.idNosocomio;
			var queryUpdateProveedorEnNosocomio = "Update Nosocomio set idProveedor = ? where idNosocomio = ?"
			var params = [idProveedor, idNosocomio]
			// actualiza el idProveedor en el nosocomio
			ejecutarQUERY_MYSQL(queryUpdateProveedorEnNosocomio, params, res, "queryUpdateProveedorEnNosocomio", "false")
		})
	}
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, "affectedRows")
}
...
exports.validarNombreProveedor = function (req, res, funcionName) {
	var tipoPersona = req.query.tipoPersona
	var nombres = req.query.nombres
	var razonSocial = req.query.razonSocial
	var idProveedor = req.query.idProveedor

	var query = "Select pr.idProveedor from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona where p.tipoPersona = ? and " +
		"CASE WHEN p.tipoPersona = 'N' then concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) = ? else p.razonSocial = ? end and pr.idProveedor!=? "

	var parametros = [tipoPersona, nombres, razonSocial, idProveedor]

	ejecutarQUERY_MYSQL(query, parametros, res, funcionName)
}
exports.validarPlacaVehiculo = function (req, res, funcionName) {
	var placa = req.query.placa
	var idVehiculo = req.query.idVehiculo
	var query = "Select v.idVehiculo from Vehiculo v where v.placa = ? and v.idVehiculo!=? "
	var parametros = [placa, idVehiculo]
	ejecutarQUERY_MYSQL(query, parametros, res, funcionName)
}
