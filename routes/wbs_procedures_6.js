...
exports.insertContratos_renovacion = function(req, res, funcionName){
	...
	var query="Select idContrato, date_format(fechaEmision, '%d/%m/%Y') as fechaEmision,  date_format(fechaVigenciaContr, '%d/%m/%Y') as fechaVigenciaContr, date_format(fechaVigenciaCert, '%d/%m/%Y') as fechaVigenciaCert, nCuotas, flota, idEmpresaTransp from Contrato where ultActualizaUsuario is null and ultActualizaFecha is null"
...
exports.insertCatsAContratos = function(req, res, funcionName){
	...
	csvConverter.fromFile(ruta_archivo,function(err,results){
		...
		var queryGetVehiculos = "Select v.idVehiculo, v.placa, ucv.prima, ucv.montoPoliza from Vehiculo v inner join UsoClaseVehiculo ucv on v.idUsoClaseVehiculo = ucv.idUsoClaseVehiculo "+
	...
	csvConverter.fromFile(ruta_archivo,function(err,results){
		for(var i=0; i<results.length; i++){
			getEmpresaTransporte(results[i], res, funcionName, function(idEmpresa, contratoRegistro){
				...
				var queryInsertContrato = "Insert into Contrato(fechaEmision, fechaVigenciaContr, fechaVigenciaCert, nCuotas, flota, idEmpresaTransp, estado, nroContratoAntiguo) values (?,?,?,?,?,?,'I',?)"
				var params = [fechaEmision, fechaInicioContrato, fechaInicioCertif, cuotas, flota, idEmpresa, nroContrato]
