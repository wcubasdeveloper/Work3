...
exports.getLiquidacionesxConcesionario = function (req, res, funcionName) {
    ...
    /*var query = "Select idLiquidacion_ventas_cabecera as idLiquidacion, nroLiquidacion, date_format(fechaLiquidacion, '%d/%m/%Y') as fechaLiquidacion, " +
        ...
        "order by idLiquidacion_ventas_cabecera ";*/
    var query = "Select v.*, if(t3.precioTotal is null, 0.00, t3.precioTotal) as precioTotal, if(t3.comisionTotal is null, 0.00, t3.comisionTotal) as comisionTotal " +
                "from (select lc.idLiquidacion_ventas_cabecera as idLiquidacion, nroLiquidacion, date_format(fechaLiquidacion, '%d/%m/%Y') as fechaLiquidacion "+
                "from Liquidacion_ventas_cabecera lc where registroEstado = 0 and (idDepositoPN = 0 or idDepositoPN is null) and idConcesionario = ? "+
                "order by idLiquidacion_ventas_cabecera) as v "+
                "LEFT JOIN (select ld.idLiquidacion_ventas_cabecera, sum(ld.precio) as precioTotal, sum(ld.comision) as comisionTotal "+
                "    from Liquidacion_ventas_detalle ld group by ld.idLiquidacion_ventas_cabecera "+
                ") t3 ON (t3.idLiquidacion_ventas_cabecera = v.idLiquidacion) ";
...
exports.imprimirOPA = function (req, res, funcionName) {
    ...
    ejecutarQUERY_MYSQL(queryDetalleOrden, params, res, funcionName, function (res, results) {
        if (results.length > 0) {
            ...
            var nroDocumentoBeneficiario = results[0].dniBeneficiario

            var placa = results[0].placa
            var numeroExpediente = results[0].idExpediente;
            ...
            var cobertura =  detallesExpediente[tipoExpediente].detalle;
            ...
            textoMonto = textoMonto + "<br><label style='font-weight:bold;'>SON: " + numerosAletras(montoEnteros).toUpperCase() + " CON " + montoDecimales + "/100 NUEVOS SOLES.</label>"
            ...
            var body = "<html>" +
                ...
                "<div style='font-size:23px; font-weight:bold; text-align:left; margin-top:15px; width:100%;'>" +
                "<img src='http://autoseguro.pe:8085/wpimages/logo_autoseguro.jpg' style='width:260px;text-align:left;' /><br><br>" +
                "</div>"+
                "<div style='font-size:21px; font-weight:bold; text-align:center; margin-top:15px;'>" +
...
'<head>' +
    '<meta charset="UTF-8">' +
    '<title>Comprobante de Pago de Indemnizaciones</title>' +
    '<style>' +
        'body {' +
...
'<body>' +
    '<div class="container">' +
        '<div class="header">' +
        '<p style="text-align:left;width:100%;padding-top:0px;margin-top:0px;margin-bottom:0px"><img src="http://autoseguro.pe:8085/wpimages/logo_autoseguro.jpg" style="width:150px;" /></p>' +
            '<p style="padding-top:0;margin-top:0px;margin-bottom:0px;margin-top:0px">ANEXO N° 6</p>' +
            '<p style="padding-top:0;margin-top:0px;margin-bottom:0px">ORDEN DE PAGO N° '+ nroOrdenPagoAgraviado +'</p>' +
            '<p style="padding-top:0;margin-top:0px;margin-bottom:0px" >COMPROBANTE DE PAGO DE INDEMNIZACIONES</p>' +
            '<p style="text-align:left;font-weight:100;padding-bottom:0px;padding-top:0px;margin-top:0px;margin-bottom:0px">Autorización del pago indemnización</p>'+
            '<p style="text-align:left;font-weight:100;padding-bottom:0px;padding-top:0px;margin-top:0px;margin-bottom:0px">Procede el pago por '+ detallesExpediente[tipoExpediente].subTitulo +' </p>'+
            '<p style="text-align:left;font-weight:100;padding-bottom:0px;padding-top:0px;margin-top:0px;margin-bottom:0px">'+ textoMonto +'</p>'+
            '<p style="text-align:left;font-weight:100;padding-bottom:0px;padding-top:0px;margin-top:0px;margin-bottom:0px">IMPORTE: '+monto+' SOLES</p>'+
            '<p style="text-align:left;font-weight:100;padding-bottom:0px;padding-top:3px;margin-top:3px;margin-bottom:0px">Se deja expresa constancia, que con el presente pago, queda liberada Autoseguro Afocat, así como el propietario y/o conductor y/o asociado del CAT, de cualquier reclamo posterior relacionado con dicho siniestro, que podría formular el beneficiario de la presente indemnización o quien sus derechos representan total y definitivamente liquidado el presente siniestro bajo todo concepto.</p>'+
            '<p style="text-align:left;font-weight:100;padding-bottom:0px;padding-top:3px;margin-top:3px;margin-bottom:0px">Asimismo, Autoseguro Afocat queda subrogada en todos los derechos y acciones que puedan corresponder contra terceros responsables, en relación con los daños de la presente indemnización</p>'+
            
            '<p style="text-align:right;font-weight:100;padding-bottom:0px;padding-top:35px;margin-top:35px;margin-bottom:0px;">'+
                '<span style="text-decoration: overline">Auditor Médico</span>'+
                '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
                '<span style="text-decoration: overline">V°B° Administración</span>'+
            '</p>'+

        '</div>' +
        '<div class="section">' +
            '<div class="box">' +
                // '<label>AUTOSEGURO AFOCAT</label>' +
                '<label style="text-align:center">INFORMACIÓN DE LA ORDEN DE PAGO, CHEQUE O TRANSFERENCIA</label>' +
                '<label>Fecha de emisión: <strong>'+ fechaAccidente +'</strong></label>' +
                '<label>Importe <strong>S/. '+ monto +'</strong></label>' +
                '<label>A la orden de: <strong>'+ nombreBeneficiario+'</strong></label>' +
                '<label>Institución Financiera: ____________________________________________</label>' +
                '<label>N° de Cuenta: ____________________________</label>' +
                '<div style="width:200px;height:60px;border:1px solid black">V°B°</div>' +
            '</div>' +
        '</div>' +

        '<div class="section">' +
            '<div class="box">' +
                '<label>Nombres y apellidos del accidentado: <strong>'+nombreAgraviado+'</strong></label>' +
                '<label>Tipo de cobertura pagada: '+detallesExpediente[tipoExpediente].detalle+'</label>' +
                '<label>Concepto del Pago: '+ detallesExpediente[tipoExpediente].subTitulo +'</label>' +
                '<label>Número del Expediente: '+ numeroExpediente +'</label>' +
            '</div>' +
        '</div>' +

        '<div class="section">' +
            '<div class="box">' +
                '<label>NOTA: Información a ser llenada por la persona que recibe la indemnización</label>' +
                '<label>Nombres y apellidos del beneficiario: '+nombreBeneficiario+'</label>' +
                '<label>Tipo de documento de identidad: '+ 'DNI'+'</label>' +
                '<label>Número del documento de identidad: '+ nroDocumentoBeneficiario +'</label>' +
                '<label>Fecha en que se recibe el pago de la indemnización: '+ $fechaImpresion +'</label>' +
                '<div class="signature-box">' +
                    '<div style="width:250px;height:60px;border:1px solid black">Firma</div>' +
                '</div>' +
            '</div>' +
        '</div>' +
...
var foorterHTML = "";
            //generatePDF(body, footer, res, "120px"); //"246px"); //anterior
            generatePDF(htmlPDF, foorterHTML, res, "10px"); //"246px"); //nuevo wcubas
...
exports.imprimirOPA_BK = function (req, res, funcionName) {
    var nroOrdenPago = req.query.nroOrden
