...
exports.generarFlujoOriginal = function(req, res, funcionName){
	...
	var queryVerificarConstante = "Select rutinaFactura from ConstantesGenerales";
	ejecutarQUERY_MYSQL(queryVerificarConstante, [], res, funcionName, function(res, resultsConstantes){

		var rutinaFactura = resultsConstantes[0].rutinaFactura
		if(rutinaFactura==1){
			...
			enviarResponse(res, ["La funcion de rutina esta desactivada : "+rutinaFactura] )
		}
	})
}
...
function unificarFacturas(req, tipoFactura, archivo, facturasAgrupadas, res, funcionName, callback){
	...
	csvConverter.fromFile(ruta_archivo,function(err, result){
		...
		for(var i=0; i<result.length; i++){
			result[i].Nosocomio=result[i].Nosocomio.trim()
			result[i].Tipo_Gasto=result[i].Tipo_Gasto.trim()
...
exports.fixPlacasEnCATS = function(req, res, funcionName){
	var queryCats = "Select nroCAT, placa from Cat where placa like '%-%'";
	ejecutarQUERY_MYSQL(queryCats, [], res, funcionName, function(res2, results2){
		console.log("cats encontrados : "+results2.length)
		for(var y=0; y<results2.length; y++){
						
			var regist = results2[y]						
			var nroCAT = regist.nroCAT
			var placa = regist.placa
			placa =  placa.replace("-","")
			placa = placa.replace(" ","")
			placa = placa.trim()
					
			var queryUpdateCats = "Update Cat set placa = ? where nroCAT = ?"
			var params = [placa, nroCAT]
						
			ejecutarQUERY_MYSQL(queryUpdateCats, params, res, "queryUpdateCats","false")
		}	
	})
}
exports.fixPlacasEnCATS_anulados = function(req, res, funcionName){
	var queryCats = "Select nroCAT, placa from Cat_anulados where placa like '%-%'";
	ejecutarQUERY_MYSQL(queryCats, [], res, funcionName, function(res2, results2){
		console.log("cats anulados encontrados : "+results2.length)
		for(var y=0; y<results2.length; y++){
			...
			var queryUpdateCats = "Update Cat_anulados set placa = ? where nroCAT = ?"
			var params = [placa, nroCAT]
...
exports.fixPlacasEnVehiculosInformados = function(req, res, funcionName){
	var query_vehiculo_informado = "Select idVehiculoInformado, placa from Vehiculos_Informados where placa like '%-%' "
	ejecutarQUERY_MYSQL(query_vehiculo_informado, [], res, funcionName, function(res4, results4){
		console.log("vehiculos encontrados: "+results4.length)
		for(var i=0; i<results4.length; i++){
			var regist = results4[i]
												
			var idVehiculoInformado = regist.idVehiculoInformado
			var placa = regist.placa
			placa =  placa.replace("-","")
			placa = placa.replace(" ","")
			placa = placa.trim()
												
			var queryUpdateVehiculos = "Update Vehiculos_Informados set placa = ? where idVehiculoInformado = ? "
			var params = [placa, idVehiculoInformado]
												
			ejecutarQUERY_MYSQL(queryUpdateVehiculos, params, res, "queryUpdateVehiculos", "false")												
		}
	})
}
exports.fixPlacasEnOtrasTablas = function(req, res, funcionName){
	var queryEvento = "Select codEvento, placaAccidente from Evento where placaAccidente like '%-%'";
	ejecutarQUERY_MYSQL(queryEvento, [], res, funcionName, function(res, results){
		for(var i=0; i<results.length; i++){
			var registro = results[i]
			var codEvento = registro.codEvento
			var placa = registro.placaAccidente
			placa =  placa.replace("-","")
			placa = placa.replace(" ","")
			placa = placa.trim()
			
			var queryUpdateEvento = "Update Evento set placaAccidente = ? where codEvento = ?"
			var params = [placa, codEvento]
			ejecutarQUERY_MYSQL(queryUpdateEvento, params, res, "queryUpdateEvento", "false")
		}
	})
}

exports.fixVehiculos = function(req, res, funcionName){
	var query="Select idVehiculo, placa from Vehiculo where placa like '%-%'";
...
function leerExcelCSV(ruta_archivo, callback){
    var Converter = require("csvtojson").Converter;
    //new converter instance
    var csvConverter=new Converter({delimiter:","});
    csvConverter.fromFile(ruta_archivo,function(err,results){
...
// Funciones para reparar las tablas de NOSOCOMIOS, PROVEEDOR Y FUNERARIAS

// Repara los nosocomios en funcion al excel subido en el direcotiro www/files/
exports.ajustarNosocomios = function(req, res, funcionName){
	req.query.nosocomios_relacionados = 0;
	req.query.nosocomios_agregar = [] // nosocomios que se agregaran
    req.query.nosocomios_a_validar = [] // nosocomios cuyo campo BORRAR es S
    var archivo = req.query.archivo
    if(archivo==null || archivo==''){
        archivo = "nosocomios_depurados.csv"
    }
    var ruta_archivo = "./www/files/"+archivo;

    leerExcelCSV(ruta_archivo, function(err, results){
        for(var i=0; i<results.length; i++){
            var nosocomio_registro = results[i]
            if(nosocomio_registro.BORRAR!=null && nosocomio_registro.BORRAR.trim() == 'S'){
                req.query.nosocomios_a_validar.push(nosocomio_registro)
            }else{
                req.query.nosocomios_agregar.push(nosocomio_registro)
            }
        }
        if(req.query.nosocomios_a_validar.length>0){
			var idNosocomios = []
            for(var i=0; i<req.query.nosocomios_a_validar.length; i++){
                idNosocomios.push(req.query.nosocomios_a_validar[i].idNosocomio.toString())
            }
			// VERIFICA SI LOS nosocomios CON CAMPO BORRAR=S estan relacionados con otras tablas
            var query = "Select n.idNosocomio, ( (select count(*) from Agraviado a where a.idNosocomio = n.idNosocomio) + " +
                "(Select count(*) from CartaGarantia cg where cg.idNosocomio = n.idNosocomio) + " +
                "(Select count(*) from Evento e where e.idNosocomio = n.idNosocomio) + "+
                "(Select count(*) from Informe i where i.idNosocomio = n.idNosocomio) +"+
				"(Select count(*) from ProyeccionGastos pg where pg.idNosocomio = n.idNosocomio) ) as totalRelaciones from Nosocomio_old n where n.idNosocomio in ("+idNosocomios+")"

            ejecutarQUERY_MYSQL(query, [], res, funcionName, function(res, resultsNosoc){
				for(var y=0; y<resultsNosoc.length; y++){
					if(resultsNosoc[y].totalRelaciones>0){ // if el nosocomio esta relacionado con otra tabla, lo agregara a la lista de nosocomios que se agregaran
						var idNosoc = resultsNosoc[y].idNosocomio
						var nosocomios_a_borrar = req.query.nosocomios_a_validar
						for(var z=0 ; z<nosocomios_a_borrar.length; z++){
							if(nosocomios_a_borrar[z].idNosocomio == idNosoc){
								req.query.nosocomios_agregar.push(nosocomios_a_borrar[z])
								req.query.nosocomios_relacionados++;
								break;
							}
						}							
					}							
                ...
                // registra
                var idNosocomios = []
                for(var y=0; y<req.query.nosocomios_agregar.length; y++){
                    idNosocomios.push(req.query.nosocomios_agregar[y].idNosocomio.toString())
                }
				
				console.log("Existen "+req.query.nosocomios_relacionados+" nosocomios relacionados!")
				
                var queryInsert = "Insert into Nosocomio select * from Nosocomio_old po where po.idNosocomio in ("+idNosocomios+") "
...
exports.reemplazarNosocomios = function(req, res, funcionName){
	// busca todos los nosocomios que se estan repitiendo
	var queryNosocomiosRepetidos = "select nombre, GROUP_CONCAT(idNosocomio ORDER BY idNosocomio SEPARATOR  ',') as idNosocomios "+
		" from Nosocomio group by nombre having count(*)>1 ";
	ejecutarQUERY_MYSQL(queryNosocomiosRepetidos, [], res, funcionName, function(res, results){
		for(var i=0; i<results.length; i++){
			var idNosocomios = results[i].idNosocomios
			var splitNosocomios = idNosocomios.split(",")
			if(splitNosocomios.length>1){
				var idNosocomioOriginal = splitNosocomios[0].trim()
				var nosocomiosAreemplazar = []
				for(var z=1; z<splitNosocomios.length; z++){
					nosocomiosAreemplazar.push(splitNosocomios[z].trim())
				}
				/*** reemplaza el nosocomio en todas las tablas relacionadas ***/
				var parametros = [idNosocomioOriginal]
				
				// reemplaza en Agraviado
				var queryAgraviado = "Update Agraviado set idNosocomio = ? where idNosocomio in ("+nosocomiosAreemplazar+") "
				ejecutarQUERY_MYSQL(queryAgraviado, parametros, res, "Agraviado", "false")
				
				// reemplaza en CartaGarantia
				var queryCartaGarantia = "Update CartaGarantia set idNosocomio = ? where idNosocomio in ("+nosocomiosAreemplazar+") "
				ejecutarQUERY_MYSQL(queryCartaGarantia, parametros, res, "CartaGarantia", "false")
				
				// reemplaza en Evento
				var queryEvento = "Update Evento set idNosocomio = ? where idNosocomio in ("+nosocomiosAreemplazar+") "
				ejecutarQUERY_MYSQL(queryEvento, parametros, res, "Evento", "false")
				
				// reemplaza en Informe
				var queryInforme = "Update Informe set idNosocomio = ? where idNosocomio in ("+nosocomiosAreemplazar+") "
				ejecutarQUERY_MYSQL(queryInforme, parametros, res, "Informe", "false")
				
				// reemplaza en ProyeccionGastos
				var queryPG = "Update ProyeccionGastos set idNosocomio = ? where idNosocomio in ("+nosocomiosAreemplazar+") "
				ejecutarQUERY_MYSQL(queryPG, parametros, res, "ProyeccionGastos", "false")
				
				// Delete nosocomio
				var queryDeleteNosocomio = "Delete from Nosocomio where idNosocomio in ("+nosocomiosAreemplazar+")"
				ejecutarQUERY_MYSQL(queryDeleteNosocomio, [], res, funcionName, "false")
...
exports.reemplazarProveedores = function(req, res, funcionName){
	// busca todos los proveedores que se estan repitiendo
	var queryProveedoresRepetidos = "Select v.nombreProveedor, GROUP_CONCAT(v.idProveedor ORDER BY v.idProveedor SEPARATOR  ',') as idProveedores "+
		"from (Select pr.idProveedor, if(p.tipoPersona = 'J',p.razonSocial, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)) as nombreProveedor "+
		"from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona) as v  group by v.nombreProveedor  having count(*)>1 "
	ejecutarQUERY_MYSQL(queryProveedoresRepetidos, [], res, funcionName, function(res, results){
		for(var i=0; i<results.length; i++){
			var idProveedores = results[i].idProveedores
			var splitProveedores = idProveedores.split(",")
			if(splitProveedores.length>1){
				var idProveedorOriginal = splitProveedores[0].trim()
				var proveedoresAreemplazar = []
				for(var z=1; z<splitProveedores.length; z++){
					proveedoresAreemplazar.push(splitProveedores[z].trim())
				}
				/*** reemplaza el proveedor original en todos las tablas relacionadas ***/
				var parametros = [idProveedorOriginal]
				// reemplazar en Guia_movimiento_cabecera
				var queryGuia = "Update Guia_movimiento_cabecera set idProveedor = ? where idProveedor in ("+proveedoresAreemplazar+") "
				ejecutarQUERY_MYSQL(queryGuia, parametros, res, "Guia_movimiento_cabecera", "false")
				
				// reemplazar en OrdenPagoProveedor
				var queryOrdenPago = "Update OrdenPagoProv set idProveedor = ? where idProveedor in ("+proveedoresAreemplazar+") "
				ejecutarQUERY_MYSQL(queryOrdenPago, parametros, res, "OrdenPagoProveedor", "false")
				
				// reemplazar en DocumentoPagoProv
				var queryDocumentoPagoProv = "Update DocumentoPagoProv set idProveedor = ? where idProveedor in ("+proveedoresAreemplazar+") "
				ejecutarQUERY_MYSQL(queryDocumentoPagoProv, parametros, res, "DocumentoPagoProv", "false")
				
				// reemplazar en Expediente
				var queryExpediente = "Update Expediente set idProveedor = ? where idProveedor in ("+proveedoresAreemplazar+") "
				ejecutarQUERY_MYSQL(queryExpediente, parametros, res, "Expediente", "false")
				
				// delete Proveedor
				var queryDeleteProveedor = "Delete from Proveedor where idProveedor in ("+proveedoresAreemplazar+")"
				ejecutarQUERY_MYSQL(queryDeleteProveedor, [], res, funcionName, "false")				
			}			
		}
		enviarResponse(res, results)
	})
}
// Repara los proveedores en funcion al excel subido en el direcotiro www/files/
exports.ajustarProveedores = function(req, res, funcionName){
    req.query.proveedores_relacionados = 0;
	req.query.proveedores_agregar = [] // proveedores que se agregaran
    req.query.proveedores_a_validar = [] // proveedores cuyo campo BORRAR es S
    var archivo = req.query.archivo
    if(archivo==null || archivo==''){
        archivo = "proveedores_depurados.csv"
    }
    var ruta_archivo = "./www/files/"+archivo;

    leerExcelCSV(ruta_archivo, function(err, results){
        for(var i=0; i<results.length; i++){
            var proveedor_registro = results[i]
            if(proveedor_registro.BORRAR!=null && proveedor_registro.BORRAR.trim() == 'S'){
                req.query.proveedores_a_validar.push(proveedor_registro)
            }else{
                req.query.proveedores_agregar.push(proveedor_registro)
            }
        }
        if(req.query.proveedores_a_validar.length>0){
			var idProveedores = []
            for(var i=0; i<req.query.proveedores_a_validar.length; i++){
                idProveedores.push(req.query.proveedores_a_validar[i].idProveedor.toString())
            }
			// VERIFICA SI LOS PROVEEDORES CON CAMPO BORRAR=S estan relacionados con otras tablas
            var query = "Select p.idProveedor, ( (select count(*) from Guia_movimiento_cabecera gc where gc.idProveedor = p.idProveedor) + " +
                "(Select count(*) from OrdenPagoProv opp where opp.idProveedor = p.idProveedor) + " +
                "(Select count(*) from DocumentoPagoProv dpp where dpp.idProveedor = p.idProveedor) + "+
                "(Select count(*) from Expediente e where e.idProveedor = p.idProveedor) ) as totalRelaciones from Proveedor_old p where p.idProveedor in ("+idProveedores+")"

            ejecutarQUERY_MYSQL(query, [], res, funcionName, function(res, resultsProv){
				for(var y=0; y<resultsProv.length; y++){
					if(resultsProv[y].totalRelaciones>0){ // if el proveedor esta relacionado con otra tabla, lo agregara a la lista de proveedores que se agregaran
						var idProv = resultsProv[y].idProveedor
						var proveedores_a_borrar = req.query.proveedores_a_validar
						for(var z=0 ; z<proveedores_a_borrar.length; z++){
							if(proveedores_a_borrar[z].idProveedor == idProv){
								req.query.proveedores_agregar.push(proveedores_a_borrar[z])
								req.query.proveedores_relacionados++;
								break;
							}
						}							
					}							
                ...
                // registra
                var idProveedores = []
                for(var y=0; y<req.query.proveedores_agregar.length; y++){
                    idProveedores.push(req.query.proveedores_agregar[y].idProveedor.toString())
                }
				
				console.log("Existen "+req.query.proveedores_relacionados+" proveedores relacionados!")
				
                var queryInsert = "Insert into Proveedor select * from Proveedor_old po where po.idProveedor in ("+idProveedores+") "
                ejecutarQUERY_MYSQL(queryInsert, [], res, funcionName, function(res, resultados){
                    enviarResponse(res, [resultados.affectedRows])
                })
            })
...
exports.repararNosocomios = function(req, res, funcionName){
	var archivo = req.query.archivo
    if(archivo==null || archivo==''){
        archivo = "correcion_nosocomios.csv"
    }
    var ruta_archivo = "./www/files/"+archivo;
    ...
    leerExcelCSV(ruta_archivo, function(err, results){
		for(var i=0; i<results.length; i++)	{
			var registro = results[i]
			var idNosocomio = registro.idNosocomio
			var nombre = registro.nombre
			var calle = registro.calle
			var idDistrito = registro.idDistrito
			var tipo = registro.tipo
			var idProveedor = registro.idProveedor
			
			var parametros = [nombre, calle, idDistrito, tipo, idProveedor, idNosocomio]
			var queryUpdate = "Update Nosocomio set nombre=?, calle=?, idDistrito=?, tipo=?, idProveedor=? where idNosocomio = ?";
