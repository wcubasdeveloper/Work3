...
var urldominio = modulo_global.urldominio; //"https://autoseguro.xyz/";
...
exports.getEventos = function (req, res, funcionName) { // Busca los eventos ingresados por la central de emergencia y la muestra en la grilla
	...
	var queryGeneral = "Select e.codEvento, e.idProcurador, c.nroCAT, c.placa,  Concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAsociado, " +
		...
		" e.choferAccidente , e.idProveedor as idNosocomio , e.idComisaria, e.idTipoAccidente, e.lugarAccidente, e.idDistrito as distritoAccidente, e.referenciaAccidente, " +
		" c.marca, c.modelo, c.annoFabricacion as anno,  date_format (c.fechaCaducidad, '%d/%m/%Y') as fechaCaducidad, i.idInforme, e.nombreContacto, e.telefonoContacto, c1.nroCAT as nroCAT1, c1.placa as placa1, Concat(pa1.nombres,' ',pa1.apellidoPaterno,' ',pa1.apellidoMaterno) as nombreAsociado1, pa1.razonSocial as razonSocial1, pa1.tipoPersona as tipoPersona1, if(e.idProcurador>0,concat(usu.Nombres,' ',usu.Apellidos), 'Sin Asignación') as nombreProcurador, e.estado, e.estadoCobertura, e.condonado, " +
		" cau1.descripcion as causal1, cau2.descripcion as causal2 from Evento e left join Cat c on e.polizaAccidente = c.nroCAT " +
		...
		" left join UsuarioIntranet usu on pro.idUsuario = usu.idUsuario " +
		"left join Causal cau1 on i.causal1=cau1.codCausal " +
		"left join Causal cau2 on i.causal2=cau2.codCausal " + queryWhere.getQueryWhere() + " order by " + orderBy + " desc";
...
exports.eliminarEvento = function (req, res, funcionName) { // elimina un evento y sus agraviados, que no tenga ningun procurador asignado.
	...
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, function (res, resultados) {
		if (resultados.length == 0) {
			...
			var queryProcedure = "call sp_eliminarEvento(?,@0)";
...
// CAMBIADO : 27/08/2018
exports.getListaNosocomios = function (req, res, funcionName) { // obtiene todos los nosocomios (C=Clinica & H=Hospital) de la Tabla PROVEEDOR, campos: idNosocomio, nombre
	var idDistrito = req.query.idDistrito;
	var arrayParametros = [idDistrito];
	var query = "Select pr.idProveedor as idNosocomio, if(p.tipoPersona='J', p.razonSocial, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)) as nombre, pr.tipoProveedor as tipo from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona where pr.tipoProveedor in ('C', 'H') and p.idDistrito = ? ";
...
exports.getCartaGarantia = function (req, res, funcionName) {
	...
	var query = "Select concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAsociado, pa.razonSocial, pa.tipoPersona,  " +
		...
		"p.edad, p.nroDocumento as DNI_Agraviado, a.idTipoAtencion as tipoAsistencia, a.diagnostico, a.montoCartaGarantia as monto, n.idProveedor as idNosocomio, if(prpe.tipoPersona='J', prpe.razonSocial, concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno)) as nombreNosocomio, " +
		"n.tipoProveedor as tipoNosocomio, prpe.idDistrito as distritoNosocomio, i.idDistritoAccidente, cg.nroCarta, date_format(cg.fecha , '%d/%m/%Y %H:%i') as fechaCarta, date_format(i.fechaHoraAccidente, '%m/%y') as fechaAccidente, cg.idServicioMedico as servicioMedico, " +
		"cg.idPrimeraProyeccion, cg.observaciones from Agraviado a " +
		"left join Informe i on a.codEvento = i.codEvento " +
		...
		"left join Persona p on a.idPersona = p.idPersona " +
		"left join Proveedor n on a.idProveedor = n.idProveedor " +
...
exports.actualizarCarta = function (req, res, funcionName) {
	...
	var diagnosticoFinal = req.query.diagnosticoFinal; // diagnostico final
	...
	ejecutarQUERY_MYSQL(queryValidarNro, paramsValida, res, funcionName, function (res1, resultados1) {
		...
		if (cantRegistros == 0) {
			...
			var queryUpdate = "Update CartaGarantia set nroCarta=?, fecha=?, idProveedor=?, idServicioMedico=?, observaciones=?, monto = ?, idTipoAtencion=? where idCarta = ?";

			var parametros = [nroCarta, fechaCarta, idNosocomio, servicioMedico, observaciones, monto, asistencia, idCarta];

			ejecutarQUERY_MYSQL(queryUpdate, parametros, res, funcionName, function (res, resultados) {
				enviarResponse(res, [resultados.affectedRows]);
				if (resultados.affectedRows > 0) {
					var queryUpdateAgraviado = "Update Agraviado set diagnostico = ?, idTipoAtencion=?, idProveedor=?, montoCartaGarantia=?, gastoTotalAprox=(SELECT sum(p.montoAproximado) from ProyeccionGastos p where p.codAgraviado='" + codAgraviado + "' and p.idProyeccionGastos!='" + idPrimeraProyeccion + "')+? " +
						" where codAgraviado = ?";
					var params = [diagnosticoFinal, asistencia, idNosocomio, monto, monto, codAgraviado];
		...
		if (cantRegistros == 0) {
			...
			ejecutarQUERY_MYSQL(queryInsertPrimeraProyeccion, paramsProy, res, funcionName, function (res, resultados) {
				...
				if (idProyeccion > 0) { // inserta la primera proyeccion
					// inserta la carta
					var queryInsert = "Insert into CartaGarantia (nroCarta, idEtapa, idTipoAtencion, estado, fecha, idProveedor, idServicioMedico, nroCAT, codEvento, codAgraviado, observaciones, monto, primeraCarta, idPrimeraProyeccion)" +
						" values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
					var parametros = [req.query.nroCarta, "1", asistencia, 'P', req.query.fechaCarta, req.query.idNosocomio, req.query.servicioMedico, req.query.nroCAT, req.query.codEvento, req.query.codAgraviado, req.query.observaciones, req.query.monto, "S", idProyeccion];
					ejecutarQUERY_MYSQL(queryInsert, parametros, res, funcionName, function (res2, resultados2) {
						...
						if (resultados2.insertId > 0) {
							var queryUpdateAgraviado = "Update Agraviado set diagnostico = ?, idTipoAtencion=?, idProveedor=?, montoCartaGarantia=?, gastoTotalAprox=gastoTotalAprox+? " +
								" where codAgraviado = ?";
							var params = [diagnosticoFinal, asistencia, idNosocomio, monto, monto, codAgraviado];
							ejecutarQUERY_MYSQL(queryUpdateAgraviado, params, res, funcionName, "false");
						}
...
function procesarEvento(req, res, funcionName, codEvento) {
	...
	var queryInsert = "Insert into Evento(codEvento, fechaAccidente, lugarAccidente, idDistrito, fechaRegistro, placaAccidente, polizaAccidente, DNIChoferAccidente, choferAccidente, " +
		"idProveedor, idComisaria, idTipoAccidente, referenciaAccidente, nombreContacto, telefonoContacto, fechaAviso) values (?,?,?,?, CURRENT_TIMESTAMP,?,?,?,?,?,?,?,?,?,?,?)";
...
exports.actualizarEvento = function (req, res, funcionName) {
	...
	var queryUpdateEvento = "Update Evento set fechaAccidente=?, lugarAccidente=?, idDistrito=?, placaAccidente=?, polizaAccidente=?, DNIChoferAccidente=?, " +
		" choferAccidente=?, idProveedor=?, idComisaria=?, idTipoAccidente=?, referenciaAccidente=?, nombreContacto=?, telefonoContacto=?, fechaAviso=? where codEvento = ?";
...
exports.getInfoCentralDeEmergencias = function (req, res, funcionName) {
	...
	var query = "Select e.codEvento, e.idProcurador, c.nroCAT, c.placa,  Concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAsociado, " +
		...
		" e.choferAccidente , e.idProveedor as idNosocomio , pr.tipoProveedor as tipoNosocomio, e.idComisaria, co.idDistrito as distritoComisaria, e.idTipoAccidente, e.lugarAccidente, e.idDistrito as distritoAccidente, e.referenciaAccidente, " +
		...
		" left join Proveedor pr on e.idProveedor = pr.idProveedor " +
	...
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, function (res, resultados) {
		if (resultados.length > 0) {
			...
			var queryAgraviados = "Select a.codAgraviado, a.codEvento, p.idPersona, p.nombres, p.apellidoPaterno, p.apellidoMaterno, p.nroDocumento as DNI, p.edad, p.telefonoMovil," +
				" a.diagnostico, a.idTipoAtencion as tipoAsistencia, a.montoCartaGarantia, a.nombreAccidente, a.edadAccidente, a.dniAccidente, pr.idProveedor as idNosocomio, pr.tipoProveedor as tipoNosocomio, if(prpe.tipoPersona='J', prpe.razonSocial, concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno)) as nombreNosocomio, prpe.idDistrito as distritoNosocomio from Agraviado a " +
				" inner join Persona p on a.idPersona = p.idPersona " +
				" left join Proveedor pr on a.idProveedor = pr.idProveedor " +
				" left join Persona prpe on pr.idPersona = prpe.idPersona " +
				" where a.codEvento = ?";
			var arrayParametrosAgraviados = [codEvento];
			ejecutarQUERY_MYSQL_Extra(resultados, queryAgraviados, arrayParametrosAgraviados, res, funcionName, function (res, rowsAgraviados, resultados) {
				resultados[0].listaAgraviados = rowsAgraviados;
...
exports.getNosocomioByNombre = function (req, res, funcionName) {
	...
	var query = "Select pr.idProveedor as idNosocomio, if(p.tipoPersona='J', p.razonSocial, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)) as nombre, pr.tipoProveedor as tipo from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona where pr.tipoProveedor in ('C', 'H') and  if(p.tipoPersona='J', p.razonSocial, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)) like '%" + nosocomio + "%' ";
	ejecutarQUERY_MYSQL(query, [], res, funcionName);
}
// CAMBIADO : 29/08/2018 (Proveedor por funeraria)
exports.getFunerariaByNombre = function (req, res, funcionName) {
	var funeraria = req.query.funeraria;
	var query = "Select pr.idProveedor as idFuneraria, if(p.tipoPersona='J', p.razonSocial, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)) as nombre from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona where pr.tipoProveedor in ('F') and  if(p.tipoPersona='J', p.razonSocial, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)) like '%" + funeraria + "%' ";
	ejecutarQUERY_MYSQL(query, [], res, funcionName);
}
...
function updateInforme(info, res, funcionName) {
	...
	var queryUpdateInforme = "Update Informe set idTipoAccidente = ?, idAsociado=?, idPropietario=?, idChofer=?, idPropietario2=?, madreChofer=?, padreChofer=?, " +
		" numheridos=?, codigoDenuncia=?, causal1=?, direccionAsociadoValida=?, idComisaria=?, calificacion=?, horaExamenCualitativo=?, horaExamenCuantitativo=?, resultadoExamenEtilico=?, " +
	...
	info.idComisaria, info.calificacionEvento, info.horaExamenCualitativo, info.horaExamenCuantitativo,
...
function insertarInforme(info, res, funcionName) { // inserta el informe, actualiza direccion del agraviado, inserta vehiculos y agraviados
	var queryInsertInforme = "Insert into Informe (codEvento, idTipoAccidente, idAsociado, idProcurador, idPropietario, idChofer, idPropietario2, madreChofer, padreChofer, " +
		"numheridos, codigoDenuncia, causal1, direccionAsociadoValida, idProveedor, idComisaria, calificacion, horaExamenCualitativo, horaExamenCuantitativo, resultadoExamenEtilico, " +
		" observaciones, idDistritoAccidente, fechaHoraAccidente, fechaHoraAviso, preguntasCalificacion, nroCAT, direccionAccidente, fechaInicioInvestigacion, fechaFinInvestigacion, UIT, sueldoMinVital) " +
		" values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, (select sueldoMinVital from ConstantesGenerales limit 1))";
	...
	abstractGuardarActualizarPersona(res, funcionName, agraviado, function (idPersona) {
		...
		ejecutarQUERY_MYSQL(queryUltimoAgraviado, [], res, funcionName, function (res, resultados) {
			...
			if (resultados.length > 0) {
				...
				numeroAgraviado = "S" + año + mes + numeroAgraviado;
				var queryInsert = "Insert into Agraviado (codAgraviado, codEvento, diagnostico, idPersona, idTipoAtencion, idProveedor, montoCartaGarantia) " +
					" values (?,?,?,?,?,?,?) ";
...
function actualizarAgraviadoInforme(agraviado, res, funcionName) {
	abstractGuardarActualizarPersona(res, funcionName, agraviado, function (idPersona) {
		var queryUpdateAgraviado = "Update Agraviado set diagnostico = ?, idPersona = ?, idTipoAtencion=?, idProveedor=?," +
...
function abstractGuardarActualizarPersona(res, funcionName, persona, callback) {
	...
	// if (persona.viene_de_servicio != undefined) {
	// 	queryAdicional = queryAdicional + ", VIENE_DE_SERVICIO = '" + persona.viene_de_servicio + "' ";
	// } else {
	// 	persona.viene_de_servicio = "";
	// }
...
exports.listaAgraviadoYProyecciones = function (req, res, funcionName) {
	...
	if (fechaDesde != "" || fechaHasta != "") {
		if (fechaDesde != "" && fechaHasta != "") {
			fechaHasta = fechaHasta + " 23:59:59";
			mquery += "where ( ev.fechaAviso between '" + fechaDesde + "' and '" + fechaHasta + "' ) ";
		} else {
			if (fechaDesde != "") {
				mquery += "where ev.fechaAviso >='" + fechaDesde + "' ";
			} else {
				fechaHasta = fechaHasta + " 23:59:59";
				mquery += "where ev.fechaAviso<='" + fechaHasta + "' ";
			}
		}
		mquery += " order by ev.fechaAviso; "
	} else {
		mquery += ";";
	}
...
exports.getProyeccionPorAgraviado = function (req, res, funcionName) {
	...
	var query = "Select a.codAgraviado, date_format(i.fechaHoraAccidente, '%d/%m/%Y %H:%i') as fechaAccidente, t.descripcion as descripcionAccidente, concat(pag.nombres,' ',pag.apellidoPaterno,' ',pag.apellidoMaterno) as nombreAgraviado, pag.nroDocumento as DNI, " +
		" a.codEvento, i.nroCAT, i.idDistritoAccidente, prpe.idDistrito as idDistritoNosocomio, c.placa, concat(paso.nombres,' ',paso.apellidoPaterno,' ',paso.apellidoMaterno) as nombreAsociado, paso.tipoPersona, paso.razonSocial, " +
		" if(prpe.tipoPersona='J', prpe.razonSocial, concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno)) as nombreNosocomio, co.nombre as nombreComisaria, a.diagnostico, a.diagnosticoAccidente as diagnosticoInicial, i.UIT from Agraviado a inner join Informe i on a.codEvento = i.codEvento " +
...
exports.getEventosConInformeCerrado = function (req, res, funcionName) { // Busca los eventos ingresados por la central de emergencia y la muestra en la grilla
	...
	var queryGeneral = "Select e.codEvento, e.estadoCobertura, i.idInforme, c.nroCAT, c.placa,  Concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAsociado, " +
		" pa.razonSocial, pa.tipoPersona, date_format (e.fechaAccidente, '%d/%m/%Y') as fechaAccidente, " +
		" i.direccionAccidente as lugarAccidente, c1.descripcion as causal1, c2.descripcion as causal2 " +
		" from Evento e inner join Informe i on e.codEvento = i.codEvento " +
		" inner join Cat c on e.nroCAT = c.nroCAT " +
		" inner join Asociado a on c.idAsociado = a.idAsociado " +
		" inner join Persona pa on a.idPersona = pa.idPersona " +
		"left join Causal c1 on i.causal1=c1.codCausal " +
		"left join Causal c2 on i.causal2=c2.codCausal " + queryWhere.getQueryWhere() + " order by e.codEvento desc";
	queryGeneral = agregarLimit(page, registrosxpagina, queryGeneral);
	ejecutarQUERY_MYSQL(queryGeneral, [], res, funcionName, function (res, resultados) {
		if (resultados.length > 0) {
			if (cantPaginas == 0) {
...
exports.getAgraviadosCarta = function (req, res, funcionName) { // 24/01/2018 : Obtiene el total de los proyectado y de las cartas en funcion a las etapas de gastos "Gastos Medicos (id=1)" y "Sepelio (id=5)"
	var codEvento = req.query.codEvento;
	var query = "Select a.codEvento, a.codAgraviado, a.diagnostico, concat (p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombreAgraviado , " +
		"p.nroDocumento, (Select sum(pg.montoAproximado) from ProyeccionGastos pg where pg.idFase in ('1', '5') and pg.codAgraviado = a.codAgraviado) as gastoTotalAprox, (select sum(cg.monto) from CartaGarantia cg where cg.codAgraviado=a.codAgraviado and cg.estado='P' and cg.idEtapa in ('1', '5')) as totalCartas from Agraviado a " +
		"inner join Persona p on a.idPersona = p.idPersona " +
		"where a.codEvento = ?";
	var arrayParametros = [codEvento];
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName);
...
exports.getListaCartasPendientes = function (req, res, funcionName) {
	var fechaDesde = req.query.fechaDesde;
	var fechaHasta = req.query.fechaHasta;
	var mquery = "select cg.idCarta,cg.nroCarta,date_format(cg.fecha, '%d/%m/%Y %H:%i') as fecha,cg.codEvento,cg.codAgraviado, " +
		"pe.razonSocial,cg.nroCAT, cg.diagnostico,a.diagnostico as diagnosticoInforme,a.diagnosticoAccidente, " +
		"concat(per.nombres,' ',per.apellidoPaterno,' ',per.apellidoMaterno) as nombreAgraviado, cg.monto " +
		"from CartaGarantia cg " +
		"inner join Agraviado a on cg.codAgraviado=a.codAgraviado " +
		"inner join Proveedor p on cg.idProveedor=p.idProveedor " +
	...
	if (fechaDesde != "" || fechaHasta != "") {
		if (fechaDesde != "" && fechaHasta != "") {
			fechaHasta = fechaHasta + " 23:59:59";
			mquery += "and ( fecha between '" + fechaDesde + "' and '" + fechaHasta + "' ) ";
		} else {
			if (fechaDesde != "") {
				mquery += "and fecha >='" + fechaDesde + "' ";
			} else {
				fechaHasta = fechaHasta + " 23:59:59";
				mquery += "and fecha<='" + fechaHasta + "' ";
			}
		}
		mquery += " order by fecha; "
	} else {
		mquery += ";";
	}
	var params = []
...
// CAMBIADO: 28 DE AGOSTO (proveedor por nosocomio)
exports.getListaCartas = function (req, res, funcionName) {
	var codEvento = req.query.codEvento;
	var codAgraviado = req.query.codAgraviado;
	var query = "Select c.idCarta, c.idEtapa as idCobertura, c.estado, concat(p.nombres,' ', p.apellidoPaterno,' ',p.apellidoMaterno) as nombreAgraviado, " +
		"p.nroDocumento as DNI, a.codAgraviado, a.codEvento, n.idProveedor as idNosocomio, if(prpe.tipoPersona='J', prpe.razonSocial, " +
		"concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno)) as nombreNosocomio, n.tipoProveedor as tipoNosocomio, " +
		"prpe.idDistrito as distritoNosocomio, c.nroCarta, date_format(c.fecha, '%d/%m/%Y') as fecha, c.idTipoAtencion as tipoAsistencia, " +
		"c.monto, c.idPrimeraProyeccion, t.descripcion as asistencia, c.nroOrdenPago from CartaGarantia c " +
		" left join Proveedor n on c.idProveedor=n.idProveedor " +
		" left join Persona prpe on n.idPersona=prpe.idPersona " +
...
// CAMBIADO: 29 DE AGOSTO (proveedor por nosocomio/funeraria)
exports.getCartaDetalle = function (req, res, funcionName) {
	var idCarta = req.query.idCarta;
	var query = "select cg.nroCarta, cg.observaciones, date_format(cg.fecha, '%d/%m/%Y %H:%i') as fechaCarta, cg.idPrimeraProyeccion, e.codEvento, date_format(e.fechaAccidente, '%d/%m/%Y %H:%i') as fechaAccidente, ta.descripcion as tipoAccidente, " +
		"n.idProveedor as idNosocomio, if(prpe.tipoPersona='J', prpe.razonSocial, concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno)) as nombreNosocomio, n.tipoProveedor as tipoNosocomio, prpe.idDistrito as distritoNosocomio, co.nombre as nombreComisaria, ca.nroCAT, concat(pe_aso.nombres,' ',pe_aso.apellidoPaterno,' ',pe_aso.apellidoMaterno) as nombreAsociado, " +
		...
		"ag.diagnostico as diagnosticoAgraviado, ag.diagnosticoAccidente as diagnosticoInicial, cg.idEtapa as idCobertura, cg.esAmpliacion, cg.idCartaAnterior, cg.idTipoAtencion as tipoAsistencia, cg.idAuditor, cg.idServicioMedico as servicioMedico, " +
...
exports.getDetalleAgraviado = function (req, res, funcionName) {
	...
	var query = "select e.codEvento, date_format(e.fechaAccidente, '%d/%m/%Y %H:%i') as fechaAccidente, ta.descripcion as tipoAccidente, " +
		...
		"pe_aso.razonSocial, pe_aso.tipoPersona, ca.placa, ag.codAgraviado, pe_ag.nroDocumento as DNI_Agraviado, " +
		"concat(pe_ag.nombres,' ',pe_ag.apellidoPaterno,' ',pe_ag.apellidoMaterno) as nombreAgraviado, " +
		"ag.diagnostico as diagnosticoAgraviado, ag.diagnosticoAccidente as diagnosticoInicial, i.UIT, i.idDistritoAccidente from Agraviado ag " +
		"inner join Evento e on ag.codEvento = e.codEvento " +
		"inner join Informe i on e.codEvento = i.codEvento " +
		"left join TipoAccidente ta on i.idTipoAccidente = ta.idTipoAccidente " +
		"left join Proveedor n on ag.idProveedor = n.idProveedor " +
		"left join Persona prpe on n.idPersona = prpe.idPersona " +
		"left join Comisaria co on i.idComisaria = co.idComisaria " +
...
exports.getListaCartasPrevias = function (req, res, funcionName) {
	var idCarta = req.query.idCarta;
	var codAgraviado = req.query.codAgraviado;
	var query = "Select idEtapa as idCobertura, idCarta, nroCarta, date_format(fecha, '%d/%m/%Y') as fecha, monto, idEtapa as idCobertura, estado, idPrimeraProyeccion from CartaGarantia where codAgraviado=? and idCarta!=? and estado!='A' order by CartaGarantia.fecha";
...
// CAMBIADO: 29/08/2018 (Proveedor por nosocomio y funeraria)
exports.editarCarta = function (req, res, funcionName) {
	...
	var servicioMedico = req.query.servicioMedico;
	var diagnosticoFinal = req.query.diagnosticoFinal;
	var observaciones = req.query.observaciones;
	var monto = req.query.monto;
	var idNosocomio = req.query.idNosocomio;
	...
	req.query.idProveedor = idNosocomio
	if (idFuneraria > 0) {
		req.query.idProveedor = idFuneraria
	}
	...
	if (nroCarta != "") { // verifica que el numero de carta no exista:
		...
		ejecutarQUERY_MYSQL(queryValidarNro, paramsValida, res, funcionName, function (res1, resultados1) {
			...
			if (cantRegistros == 0) { // el numero de carta esta disponible
				var query = "Update CartaGarantia set idEtapa=?, fecha=?, nroCarta=?, estado=?, idProveedor=?, esAmpliacion=?, idCartaAnterior=?, idTipoAtencion=?, idAuditor=?, idServicioMedico=?, observaciones=?, monto=? where idCarta=?";
				var parametros = [idEtapa, fechaCarta, nroCarta, estado, idProveedor, ampliacion, cartaPrevia, tipoAsistencia, auditor, servicioMedico, observaciones, monto, idCarta];
				ejecutarQUERY_MYSQL(query, parametros, res, funcionName, function (res, resultados) {
					var filasAfectadas = [resultados.affectedRows];
					enviarResponse(res, filasAfectadas);

					// actualiza diagnostico del agraviado
					var diagnosticoFinal = req.query.diagnosticoFinal;
					var codAgraviado = req.query.codAgraviado;
					var params = [diagnosticoFinal, codAgraviado]
					var queryUpdateAgraviado = "update Agraviado set diagnostico=? where codAgraviado=?"
					ejecutarQUERY_MYSQL(queryUpdateAgraviado, params, res, funcionName, "false");
...
exports.guardarCarta = function (req, res, funcionName) {
	...
	if (nroCarta != "") { // verifica que el numero de carta no exista:
		...
		ejecutarQUERY_MYSQL(queryValidarNro, paramsValida, res, funcionName, function (res1, resultados1) {
			var cantRegistros = resultados1[0].cantidad;
			if (cantRegistros == 0) { // el numero de carta esta disponible
				//Actualiza el diagnostico del agraviado
				var queryUpdateAgraviado = "Update Agraviado set diagnostico=? where codAgraviado = ?";
				var params = [diagnosticoFinal, codAgraviado]
				ejecutarQUERY_MYSQL(queryUpdateAgraviado, params, res, funcionName, function (res, resultados) {
					// Registra la carta de garantia
					var query = "Insert into CartaGarantia(idEtapa, fecha, nroCarta, idTipoAtencion, idProveedor, esAmpliacion, idServicioMedico, nroCAT, codEvento, codAgraviado, " +
						" observaciones, monto, idCartaAnterior, idAuditor ) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
					var parametros = [idEtapa, fechaCarta, nroCarta, tipoAsistencia, idProveedor, ampliacion, servicioMedico, nroCAT, codEvento, codAgraviado, observaciones, monto, cartaPrevia, auditor];
...
//15/JUL/2019 cambio de tabla Gasto x OrdenPagoProv, OrdenPago x DocumentoPagoProv
exports.getTotalFacturaPorEtapa = function (req, res, funcionName) {
	var codAgraviado = req.query.codAgraviado
	var query = "select (select sum(f1.monto) from DocumentoPagoProv f1 where f1.codAgraviado=ag.codAgraviado and f1.idEtapa='1') as monto1, " +
		"(select sum(f2.monto) from DocumentoPagoProv f2 where f2.codAgraviado=ag.codAgraviado and f2.idEtapa='5') as monto5 " +
		" from Agraviado ag where ag.codAgraviado=?";
	var params = [codAgraviado];
	ejecutarQUERY_MYSQL(query, params, res, funcionName);
}
exports.getTotalOrdenesPorEtapa = function (req, res, funcionName) {
	var codAgraviado = req.query.codAgraviado
	var query = "select " +
		"(select sum(o1.monto) from OrdenPagoAgraviado o1 where o1.codAgraviado=ag.codAgraviado and o1.idEtapa='1' and o1.estado in ( 'B', 'P' )) as monto1, " +
		"(select sum(o2.monto) from OrdenPagoAgraviado o2 where o2.codAgraviado=ag.codAgraviado and o2.idEtapa='2' and o2.estado in ( 'B', 'P' )) as monto2, " +
		"(select sum(o3.monto) from OrdenPagoAgraviado o3 where o3.codAgraviado=ag.codAgraviado and o3.idEtapa='3' and o3.estado in ( 'B', 'P' )) as monto3, " +
		"(select sum(o4.monto) from OrdenPagoAgraviado o4 where o4.codAgraviado=ag.codAgraviado and o4.idEtapa='4' and o4.estado in ( 'B', 'P' )) as monto4, " +
		"(select sum(o5.monto) from OrdenPagoAgraviado o5 where o5.codAgraviado=ag.codAgraviado and o5.idEtapa='5' and o5.estado in ( 'B', 'P' )) as monto5 " +
		"from Agraviado ag where ag.codAgraviado=?";
	/*var query = "select (select sum(o1.monto) from OrdenPagoAgraviado o1 where o1.codAgraviado=ag.codAgraviado and o1.idEtapa='1' and o1.estado in ( 'B', 'P' )) as monto1, "+
						"(select sum(o2.monto) from OrdenPagoAgraviado o2 where o2.codAgraviado=ag.codAgraviado and o2.idEtapa='5' and o2.estado in ( 'B', 'P' )) as monto5 "+
				" from Agraviado ag where ag.codAgraviado=?";*/
	var params = [codAgraviado];
	ejecutarQUERY_MYSQL(query, params, res, funcionName);
}
...
// FIN DE CUS 02
// CAMBIADO: 29/08/2018 (Proveedor por nosocomio)
exports.getListaFunerarias = function (req, res, funcionName) { // obtiene todos los tipos de las funerarias

	var query = "Select pr.idProveedor as idFuneraria, if(p.tipoPersona='J', p.razonSocial, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)) as nombre from Proveedor pr inner join Persona p on pr.idPersona = p.idPersona where pr.tipoProveedor in ('F') ";
	var parametros = [];
	...
	switch (tipoCarta) {
		case '1': // carta de garantia de gastos
			...
			var body =
				...
				"CARTA DE GARANTIA (nro Ref)" + nroCarta +
				"</div>" +
				"<div>" +
				"<br><br>" +
				...
				"<td style='text-align:left; font-weight:bold; font-size:12px;'>Señor(es):</td>" +
				"<td style='text-align:left; font-size:12px;'>" + nosocomio_funeraria + "</td>" +
				"</tr>" +
				"</table>" +
				"<br>" +
				"<table style='width:" + ancho + "; margin:auto; font-style:" + font + ";' >" +
				...
				"<td colspan='2' style='text-align:left; padding-left:20px; font-weight:bold; font-size:12px;'>Referencia del Siniestro: </td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; padding-left:46px;; font-weight:bold; font-size:12px;'>Asociado(a): </td>" +
				"<td style='text-align:left; font-size:12px;'>" + asociado + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; padding-left:46px;; font-weight:bold; font-size:12px;'>Paciente: </td>" +
				"<td style='text-align:left; font-size:12px;'>" + paciente + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; padding-left:46px;; font-weight:bold; font-size:12px;'>Vehiculo Placa de Rodaje: </td>" +
				"<td style='text-align:left; font-size:12px;'>" + placa + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; padding-left:46px;; font-weight:bold; font-size:12px;'>Diagnóstico: </td>" +
				"<td style='text-align:left; font-size:12px;'>" + diagnostico + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; padding-left:46px;; font-weight:bold; font-size:12px;'>Fecha de Ocurrencia: </td>" +
				"<td style='text-align:left; font-size:12px;'>" + fechaOcurrencia + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; padding-left:46px;; font-weight:bold; font-size:12px;'>Certificado contra accidente de tránsito: </td>" +
				"<td style='text-align:left; font-size:12px;'>" + cat + "</td>" +
				"</tr>" +
				"</table>" +
				"<br>" +
			...
			html =
				...
				"</head>" +
				"<body style='padding-top:0px; background-color:; zoom: 75%;'>" + //Ojo cambio modulo (html-pdf)
				"<br>" +
		...
		case '2': // carta de garantia por sepelio
			...
			body =
				"<div style='width:" + ancho + "; font-size:12px; background-color:; min-height:40px; font-style:" + font + ";'>" +
				...
				"<br>" +
				"<br>" +
				"<table id='datos_impresion' style='float:left; width:350px; font-size:12px; color:black; background-color:;'>" +
				"<col style='width:45%'>" +
				"<col style='width:55%'>" +
				"<tr>" +
				...
				"<br>" +
				"<br>" +
				"<table style='width:" + ancho + "; font-size:12px; color:black; background-color:;'>" +
				"<col style='width:6%'>" +
				"<col style='width:14%'>" +
				"<col style='width:27%'>" +
				"<col style='width:11%'>" +
				...
				"</table>" +
				"<br>" +
				"<p style='text-align:justify; font-size:14px; line-height:15px;'>" +
				...
				"<br>" +
				"<br>" +
				"<p style='text-align:justify; font-size:12px; line-height:15px;'>" +
...
function crearCartaPDF(resultSet, req, res, funcionName) {
	...
	var diagnosticoCarta = resultSet.diagnosticoCarta;
	var observaciones = resultSet.observaciones
	var diagnosticoAccidente = resultSet.diagnosticoAccidente
	...
	switch (tipoCarta) {
		case '1': // carta de garantia de gastos
			...
			var body =
				"<div style='width:" + ancho + ";font-family:arial; background-color:; min-height:40px; font-style:" + font + ";'>" +

				"<!--logo autoseguro a la izquierda -->" +
				"<div style='float:left; width:260px; background-color:;'>" +
				"<img src='http://autoseguro.pe:8085/wpimages/logo_autoseguro.jpg' style='width:260px;'/>" +
				//"<img src='http://sistema.autoseguro.pe/wpimages/logo_autoseguro.jpg" + urldominio + "wpimages/logo_autoseguro.jpg' style='width:260px;'/>" +
				"</div>" +
				"<div style='float:right; width:250px; background-color:;'>" +
				"<table style='width:280px; font-size:17px; !important'>" +
				"<col style='width:50%'>" +
				"<col style='width:50%'>" +
				"<tr>" +
				"<td style='text-align:right;font-weight:bold;'>F. de Accidente: </td>" +
				"<td style='font-size:14px;'>" + fechaOcurrencia + "</td>" +
				"</tr>" +
				"<tr>" +
				"<td style='text-align:right;font-weight:bold;'>Cod Evento: </td>" +
				"<td style='font-size:14px;'>" + codEvento + "</td>" +
				"</tr>" +
				"<tr>" +
				"<td style='text-align:right;font-weight:bold;'>Nro Siniestro: </td>" +
				"<td style='font-size:14px;'>" + codAgraviado + "</td>" +
				"</tr>" +
				"</table>" +
				"</div>" +
				"<br><br><br><br><br><br><br><br>" +

				"<div id='Titulo' style='font-size:20px; font-weight:bold; width:" + ancho + "; text-align:center;'>" +
				"CARTA DE GARANTIA : " + nroCarta +
				"</div>" +
				"<br><br>" +
				"<table style='width:" + ancho + "; margin:auto; font-size:17px !important;font-style:" + font + ";' >" +
				"<col style='width:10%'>" +
				"<col style='width:90%'>" +
				"<tr style='height:13px;'>" +
				"<td style='text-align:left; font-weight:bold;'>Señor(es):</td>" +
				"<td style='text-align:left; '>" + nosocomio_funeraria + "</td>" +
				"</tr>" +
				"</table>" +
				"<br>" +
				"<table style='width:" + ancho + "; margin:auto;font-size:17px !important; font-style:" + font + ";' >" +
				...
				"<td colspan='4' style='text-align:left; font-weight:bold;'>Referencia del Siniestro: </td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; font-weight:bold;'>F. de Atención: </td>" +
				"<td style='text-align:left; '>" + fechaAtencion + "</td>" +
				"<td style='text-align:left; font-weight:bold;'>Placa: </td>" +
				"<td style='text-align:left; '>" + placa + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; font-weight:bold; '>Nro CAT: </td>" +
				"<td style='text-align:left; '>" + cat + "</td>" +
				"<td style='text-align:left; font-weight:bold; '>Asegurado: </td>" +
				"<td style='text-align:left; '>" + asociado + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; font-weight:bold; '>Especialidad: </td>" +
				"<td style='text-align:left; '>" + servicioMedico + "</td>" +
				"<td style='text-align:left; font-weight:bold; '>Paciente: </td>" +
				...
				"<col style='width:71%'>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; font-weight:bold;   padding-left: 16px;'>Diagnóstico Inicial</td>" +
				"<td style='text-align:left; font-weight:bold;  '>:</td>" +
				"<td style='text-align:left;  '>" + diagnosticoAccidente + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; font-weight:bold;   padding-left: 16px;'>Diagnóstico Final</td>" +
				"<td style='text-align:left; font-weight:bold;  '>:</td>" +
				"<td style='text-align:left;  '>" + diagnostico + "</td>" +
				"</tr>" +
				"<tr style='height:18px;'>" +
				"<td style='text-align:left; font-weight:bold;   padding-left: 16px;'>Observaciones</td>" +
				"<td style='text-align:left; font-weight:bold;  '>:</td>" +
				...
				"<br>" +
				"<div id='cuerpo' style='width:" + ancho + "; margin:auto;'>" +
				"<p style='text-align:justify; font-size:17px; line-height:18.5px; padding-top:7px;'>" +
				"De nuestra especial consideracion: " +
				"</p>" +
				"<p style='text-align:justify; font-size:18px; line-height:18.5px; padding-top:7px;'>" +
				...
				"<table style='width:" + ancho + "; margin:auto; font-style:" + font + "; font-size:17px !important;' border=1>" +
				"<col style='width:33%'>" +
				"<col style='width:33%'>" +
				"<col style='width:34%'>" +
				...
				"</tr>" +
				"</table>" +
				"<p style='text-align:justify; font-size:17px; line-height:18.5px; padding-top:7px; font-weight:bold;'>" +
			...
			html =
				...
				"<meta content='text/html; charset=utf-8' http-equiv='Content-Type'>" +
				"</head>" +
				//"<body style='padding-top:0px; background-color:; '>" + *OjO uso de 'html-pdf.js'
				"<body style='padding-top:0px; background-color:; zoom: 85%;'>" +
				"<br>" +
				"<div style='margin:auto; width:" + ancho + "; text-align:justify; margin-top:4px; background-color:;'>" +
				body +
				"</div>" +
				"</body>" +
				"</html>";
		...
		case '2': // carta de garantia por sepelio
			...
			var ancho = "835px";
			var fechaPrint = req.query.fechaPrint;
			var header = "<h2>This is the header</h2>";
			body =
				"<div style='width:" + ancho + "; font-family:arial; font-size:12px; background-color:; min-height:40px; font-style:" + font + ";'>" +
				"<table id='datos_impresion' style='float:right; width:250px; color:gray; font-size:9px; background-color:;'>" +
				"<col style='width:43%'>" +
				"<col style='width:2%'>" +
				"<col style='width:55%'>" +
				...
				"<br>" +
				"<br>" +
				"<table id='datos_impresion' style='float:left; width:350px; font-size:14px; color:black; background-color:;'>" +
				"<col style='width:45%'>" +
				"<col style='width:55%'>" +
				"<tr>" +
				"<td style='font-weight:bold;'>NRO DE CARTA SEPELIO</td>" +
				...
				"<br>" +
				"<br>" +
				"<table style='width:" + ancho + "; font-size:14px; color:black; background-color:;'>" +
				"<col style='width:6%'>" +
				"<col style='width:14%'>" +
				"<col style='width:27%'>" +
				"<col style='width:11%'>" +
				...
				"<br>" +
				"<br>" +
				"<p style='text-align:center; font-size:14px; line-height:15px;'>" +
				"-----------------------------------------------" +
				"<br>" +
				nombreUsuario +
				"<br>" +
			...
			html =
				...
				"<meta content='text/html; charset=utf-8' http-equiv='Content-Type'>" +
				"</head>" +
				//"<body style='padding-top:0px; background-color:; '>" + //*OJO cambio modulo (html-pdf)
	...
	//generatePDF1(header,html, footer, res, "88px");
	generatePDF(html, footer, res, "88px");
}
// CAMBIADO: 28 DE AGOSTO (proveedor por nosocomio)
exports.generarCartaGarantia = function (req, res, funcionName) { // Genera una carta de garantia (1=gastos , 2=sepelio)
	...
	if (idCarta != undefined) {
		var query = "select date_format(cg.fecha, '%d/%m/%Y %H:%i') as fechaCarta, cg.idPrimeraProyeccion, e.codEvento, date_format(e.fechaAccidente, '%d/%m/%Y') as fechaAccidente, ta.descripcion as tipoAccidente, " +
			" n.idProveedor as idNosocomio, if(prpe.tipoPersona='J', prpe.razonSocial, concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno)) as nombreNosocomio, n.tipoProveedor as tipoNosocomio, prpe.idDistrito as distritoNosocomio, co.nombre as nombreComisaria, ca.nroCAT, concat(pe_aso.nombres,' ',pe_aso.apellidoPaterno,' ',pe_aso.apellidoMaterno) as nombreAsociado, " +
			"pe_aso.razonSocial, pe_aso.tipoPersona, ca.placa, ag.codAgraviado, pe_ag.nroDocumento as DNI_Agraviado, " +
			"concat(pe_ag.nombres,' ',pe_ag.apellidoPaterno,' ',pe_ag.apellidoMaterno) as nombreAgraviado, " +
			"ag.diagnostico as diagnosticoAgraviado, ag.diagnosticoAccidente, cg.idEtapa as idCobertura, cg.esAmpliacion, tip.descripcion as tipoAsistencia, cg.idAuditor, sm.descripcion as servicioMedico, " +
			"cg.diagnostico as diagnosticoCarta, cg.observaciones, cg.monto, i.UIT, i.idDistritoAccidente from CartaGarantia cg " +
			"inner join Evento e on cg.codEvento = e.codEvento " +
			"inner join Informe i on e.codEvento = i.codEvento " +
			"left join TipoAccidente ta on i.idTipoAccidente = ta.idTipoAccidente " +
			"inner join Agraviado ag on cg.codAgraviado = ag.codAgraviado " +
		...
		ejecutarQUERY_MYSQL(query, parametros, res, funcionName, function (res, resultados) {
			if (resultados.length > 0) {
				...
				var resultSet = {
					...
					"diagnosticoCarta": resultados[0].diagnosticoCarta,
					"observaciones": resultados[0].observaciones,
					"diagnosticoAccidente": resultados[0].diagnosticoAccidente
				};
...
exports.getListaNosocomiosPorIdNosocomio = function (req, res, funcionName) { // obtiene los registros de los Nosocomio de un distrito de otro nosocomio previo (USADO EN CUS 03)
	...
	var query = "Select p.idProveedor as idNosocomio, p.tipoProveedor as tipo, if(prpe.tipoPersona='J', prpe.razonSocial, concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno)) as nombre " +
		" from Proveedor p inner join Persona prpe on p.idPersona = prpe.idPersona where prpe.idDistrito in (select pe.idDistrito from Proveedor p2 inner join Persona pe on p2.idPersona = pe.idPersona where p2.idProveedor = ?) " +
		" order by if(prpe.tipoPersona='J', prpe.razonSocial, concat(prpe.nombres,' ',prpe.apellidoPaterno,' ',prpe.apellidoMaterno))";
	...
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, function (res, resultados) {
		...
		ejecutarQUERY_MYSQL_Extra(resultados, queryAgraviados, arrayParametrosAgraviados, res, funcionName, function (res, rowsAgraviados, resultados) {
			...
			ejecutarQUERY_MYSQL_Extra(resultados, queryVehiculos, arrayParams, res, funcionName, function (res, rowsVehiculos, resultados) {
				...
				variables.push({ 'nombreVariable': 'placa', 'value': placa });
				...
				var tablaAgraviados = '<table border="1" id="tabla_agraviados" width="100%" style="font-size:12px;">' +
					'<thead>' +
					'  <tr style="color:white; background-color:#4485A6; font-size:12px; height:25px;">' +
				...
				fs.readFile(ruta_archivo, (err, data) => {
					if (err) {
						...
						var xid1 = html.indexOf("idDILIGENCIAS");
						var top1 = html.indexOf("top", xid1);
						var mleftHTML1 = html.substring(0, top1 + 4);
						var mrightHTML1 = html.substring(top1 + 7);
						var topDiligencias = 420 + (listaAgraviados.length - 1) * 35;
						html = mleftHTML1 + topDiligencias.toString() + mrightHTML1
						var xid2 = html.indexOf("idOBSERVACIONES");
						var top2 = html.indexOf("top", xid2);
						var mleftHTML2 = html.substring(0, top2 + 4);
						var mrightHTML2 = html.substring(top2 + 7);
						var topObservaciones = topDiligencias + 450;
						html = mleftHTML2 + topObservaciones.toString() + mrightHTML2
...
exports.verificarCartaPrevia = function (req, res, funcionName) { // verifica si la carta esta registrada como una carta previa. (10/08/2017)
	var idCarta = req.query.idCarta;
	var codEvento = req.query.codEvento;
	var codAgraviado = req.query.codAgraviado;
	var parametros = [idCarta, codEvento, codAgraviado];
	var query = "Select idCarta, nroCarta, primeraCarta from CartaGarantia where idCartaAnterior = ? and codEvento=? and codAgraviado=?";
	ejecutarQUERY_MYSQL(query, parametros, res, funcionName);
}
exports.terminarEvento = function (req, res, funcionName) {
	var codEvento = req.query.codEvento;
	var idUsuarioUpdate = req.query.idUsuarioUpdate;
	var motivo = req.query.motivo;
	var arrayParametros = [motivo, idUsuarioUpdate, codEvento]
	var query = "Update Evento set estadoCobertura='F', detalleFinCobertura = ?, fechaFinCobertura=now(), idUsuarioFinCobertura=? where codEvento = ?";
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, "affectedRows");
}
exports.revertirEvento = function (req, res, funcionName) {
	var codEvento = req.query.codEvento;
	var idUsuarioUpdate = req.query.idUsuarioUpdate;
	var arrayParametros = [idUsuarioUpdate, codEvento]
	var query = "Update Evento set estadoCobertura='C', detalleFinCobertura = 'Revertido a (C)oberturable', fechaFinCobertura=now(), idUsuarioFinCobertura=? where codEvento = ?";
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, "affectedRows");
}
exports.listarFacturasYordenes = function (req, res, funcionName) {
	var codAgraviado = req.query.codAgraviado
	var idEtapa = req.query.idEtapa
	var query = "select * from ((select concat('N° IPRESS ', g.nroDocumento) as numero, g.idEtapa, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, " +
		" g.monto from OrdenPago g where g.codAgraviado = ? and g.idEtapa=? and g.estado in ( 'B', 'P' )) UNION ALL (select concat('N° IPRESS ', f.nroDocumento) as numero, " +
		" f.idEtapa, DATE_FORMAT (f.fechaDoc, '%d/%m/%Y') as fechaDoc, f.monto from Gasto f where f.codAgraviado=? and f.idEtapa=?)) as v order by STR_TO_DATE(v.fechaDoc, '%d/%m/%Y')";

	var parametros = [codAgraviado, idEtapa, codAgraviado, idEtapa]
	ejecutarQUERY_MYSQL(query, parametros, res, funcionName);
}
exports.totalFacturasYordenes = function (req, res, funcionName) {
	var codAgraviado = req.query.codAgraviado
	var query = "select * " +
		"from ( " +
		"    (select concat('N° IPRESS ', g.nroDocPago) as numero, DATE_FORMAT (g.fechaAprobacion, '%d/%m/%Y') as fechaDoc,g.monto,g.idEtapa " +
		"from OrdenPagoAgraviado g where g.codAgraviado = ? and g.estado in ( 'B', 'P' )) " +
		"UNION ALL " +
		"(select concat('N° IPRESS ', f.nroDocumento) as numero, DATE_FORMAT (f.fechaEmision, '%d/%m/%Y') as fechaDoc, f.monto , f.idEtapa " +
		"from DocumentoPagoProv f where f.codAgraviado=? )) as v " +
		"order by STR_TO_DATE(v.fechaDoc, '%d/%m/%Y');";
	/*        "select * from ((select concat('N° IPRESS ', g.nroDocumento) as numero, g.idEtapa, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, "+
			" g.monto from OrdenPago g where g.codAgraviado = ? and g.estado in ( 'B', 'P' )) UNION ALL (select concat('N° IPRESS ', f.nroDocumento) as numero, "+
			" f.idEtapa, DATE_FORMAT (f.fechaDoc, '%d/%m/%Y') as fechaDoc, f.monto from Gasto f where f.codAgraviado=? )) as v order by STR_TO_DATE(v.fechaDoc, '%d/%m/%Y')";
	*/
	...
	csvConverter.fromFile(ruta_archivo, function (err, result) {
		for (var i = 0; i < result.length; i++) {
			getIdProveedor(result[i].Nosocomio, i, res, funcionName, function (data) {
				...
				result[index].monto = result[index].Monto_Factura;
...
function registrarFactura(re, resp) {
	var queryInsert = "Insert into Gasto(fechaDoc, idProveedor, idTipoDocumento, nroDocumento, monto, idEtapa, codAgraviado, codEvento, codigoEnlace, origen) values (?,?,?,?,?,?,?,?,?,?)";
	//Para las Cartas de Garantia nuevas el codigoEnlace esta en blanco y se debe llenar con el idCartaGarantia, sino se mantiene el codigoEnlace original
	var origen = 0;
	var ncodigoEnlace = re.id_carta_garantia;
	if (re.codigoEnlace != "") {
		ncodigoEnlace = re.codigoEnlace
		origen = 1;
	}
	re.ncodigoEnlace = ncodigoEnlace;
	var arrayParametros = [re.fecha, re.idProveedor, re.idTipoDocumento, re.Nro_Documento_Factura_Ipress, re.monto, re.idEtapa, re.Cod_Agraviado, re.evento, ncodigoEnlace, origen];
	console.log("pasarFactura>> Insertando Factura=" + re.Nro_Documento_Factura_Ipress + ", codigoEnlace=" + ncodigoEnlace);
	ejecutarQUERY_MYSQL_Extra(re, queryInsert, arrayParametros, resp, "registrarFacturas", function (res, resultados, datos) {
		var filasAfectadas = resultados.affectedRows;
		if (filasAfectadas > 0) {
			// actualiza el estado de la carta a "Facturada (F)" si el monto total de sus facturas cubre el monto de la carta, sino lo cambia a "R=Facturada Parcial"
			var queryActualizaCartasPendientes = "Update CartaGarantia cg set cg.estado = if((select sum(f.monto) from Gasto f where f.codigoEnlace = IF(cg.origen = 0, cg.idCarta, cg.codigoEnlace) and f.origen=cg.origen) >= cg.monto, 'F', if((select sum(f.monto) from Gasto f where f.codigoEnlace = IF(cg.origen = 0, cg.idCarta, cg.codigoEnlace) and f.origen=cg.origen)>0, 'R', 'P')) where cg.estado in ('P', 'R') and cg.codigoEnlace = '" + datos.ncodigoEnlace + "' ";

			ejecutarQUERY_MYSQL(queryActualizaCartasPendientes, [], res, "actualizaEstadoCartas", function (res, results) {
				var filasAfectadas = results.affectedRows;
				console.log("Se registro la factura. " + filasAfectadas + " cartas fueron actualizadas.");
			})
		} else {
			console.log("No se pudo registrar la factura con codigo de enlace : " + datos.ncodigoEnlace);
		}
	});
}
...
exports.cambiarEstadoCarta = function (req, res, funcionName) {
	var idCarta = req.query.idCarta
	var estado = req.query.estado

	var query = "Update CartaGarantia set estado = ? where idCarta = ?"
	var parametros = [estado, idCarta]
	ejecutarQUERY_MYSQL(query, parametros, res, funcionName, "affectedRows")
}