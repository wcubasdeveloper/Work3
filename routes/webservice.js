...
var urldominio = modulo_global.urldominio; //"http://autoseguro.xyz/";
/* ***************************************************** */
...
exports.actualizarDatosAccidente = function (req, res, funcionName) {
	...
	var query = "call sp_actualizarDatosAccidente(?,?,?,?,?,?,?,?,?,?,@0)";
...
exports.guardarDatosActores = function (req, res, funcionName) { // LISTO
	...
	var nombrePropietarioC = req.query.nombrePropietarioC;
	var apePatPropietarioC = req.query.apePatPropietarioC;
	var apeMatPropietarioC = req.query.apeMatPropietarioC;
	var nroDocPropietarioC = req.query.nroDocPropietarioC;
	...
	if (idPersonaPropietario > 0) { // ya existe la persona
		...
		if (idPersonaPropietario != idPersonaAsociado) { // si es diferente al asociado actualizara los datos de la persona
			...
			queryAboutPropietario = "update Persona set nombres=?, apellidoPaterno=?, apellidoMaterno=?, razonSocial=?," +
				...
				"sector=?, referencia=?, telefonoFijo=?, telefonoMovil=?, conyugeNombres=?, conyugeApellidoPaterno=?, conyugeApellidoMaterno=?, conyugeNroDocumento=? where idPersona=?";
			arrayParametrosPropietario = [nombrePropietario, apePatPropietario,
				...
				telmovilPropietario, nombrePropietarioC, apePatPropietarioC,
				apeMatPropietarioC, nroDocPropietarioC, idPersonaPropietario];
		}
		...
		queryAboutPropietario = "insert INTO Persona (nombres, apellidoPaterno, apellidoMaterno, tipoPersona, razonSocial, nroDocumento, fechaNacimiento, idDistrito, " +
			"telefonoFijo, telefonoMovil,  calle, nro, mzLote, sector, referencia, fechaRegistro, conyugeNombres, conyugeApellidoPaterno, conyugeApellidoMaterno, conyugeNroDocumento) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,?,?,?,?)";
		arrayParametrosPropietario = [nombrePropietario, apePatPropietario, apeMatPropietario, tipoPropietario, razonSocialPropietario, nroDocPropietario, fecNacPropietario, idDistritoPropietario,
			telfijoPropietario, telmovilPropietario, callePropietario, nroPropietario, lotePropietario, sectorPropietario, referenciaPropietario, nombrePropietarioC, apePatPropietarioC,
			apeMatPropietarioC, nroDocPropietarioC];
	}
...
exports.actualizarAgraviado = function (req, res, funcionName) {
	...
	// ACTUALIZA AGRAVIADO
	var query = "call sp_actualizarAgraviado(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,@0)";
...
exports.getAllNotificaciones = function (req, res, funcionName) {
	...
	var query = "select n.idNotificacion, n.codEvento, DATE_FORMAT (n.fechaEmision, '%d/%m/%Y') as fechaEmision, n.descripcionBreve, n.estado, n.resultado, m.descripcion as motivo, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombreAsociado, p.tipoPersona, p.razonSocial, n.idMotivo, n.medio, n.contenidoNotificacion from Notificacion n " +
...
exports.getEventosGenerales = function (req, res, funcionName) { // FALTCOMP
	...
	if (tipoBusqueda == "A") {
		...
		query = "SELECT e.codEvento, e.estado, e.condonado, e.nroCAT , c.placa, date_format (e.fechaAccidente, '%d/%m/%Y %H:%i:%s') as fechaAccidente, " +
			"e.lugarAccidente, ta.descripcion as descripcionEvento, de.nombre as distritoEvento, pp.idPersona as idPersonaPropietario, " +
			"pp.nombres as nombresPropietario, pp.apellidoPaterno as apellidoPaternoPropietario, pp.apellidoMaterno as apellidoMaternoPropietario, " +
			"pp.tipoPersona as tipoPropietario, pp.razonSocial as razonPropietario, " +
			"(concat(pp.calle,' ',pp.nro,' ',pp.mzLote,' ',pp.sector)) as direccionPropietario, " +
			"pp.telefonoFijo as telefonoFijoPropietario, pp.telefonoMovil as celularPropietario, dp.nombre as distritoPropietario, pc.idPersona as idPersonaChofer, " +
			"pc.nombres as nombresChofer, pc.apellidoPaterno as apellidoPaternoChofer, pc.apellidoMaterno as apellidoMaternoChofer, pc.nroDocumento as dniChofer, " +
			"(concat(pc.calle,' ',pc.nro,' ',pc.mzLote,' ',pc.sector)) as direccionChofer, pc.telefonoFijo as telefonoFijoChofer, pc.telefonoMovil as celularChofer, " +
			"dc.nombre as distritoChofer, pa.idPersona as idPersonaAsociado, pa.nombres as nombresAsociado, pa.apellidoPaterno as apellidoPaternoAsociado, " +
			"pa.apellidoMaterno as apellidoMaternoAsociado, pa.tipoPersona as tipoAsociado, pa.razonSocial, " +
			"(concat(pa.calle,' ',pa.nro,' ',pa.mzLote,' ',pa.sector)) as direccionAsociado, pa.telefonoFijo as telefonoFijoAsociado, " +
			"pa.telefonoMovil as celularAsociado, da.nombre as distritoAsociado, c1.descripcion as causal1, c2.descripcion as causal2, ch.licenciaChofer " +
			"from Evento e " +
			"left join Propietario prop on e.idPropietario=prop.idPropietario " +
			"left join Persona pp on prop.idPersona=pp.idPersona " +
			"left join Chofer ch on e.idChofer=ch.idChofer " +
...
exports.getEventosGeneralesDomicilioLegal = function (req, res, funcionName) { // FALTCOMP
	...
	if (tipoBusqueda != undefined) {
		if (tipoBusqueda == "F") { // fecha                
		...
		} else if (tipoBusqueda == "A" || tipoBusqueda == "G") { // Agraviado x Nombres,DNI o solo CODIGO
			var nombreAgraviado = (req.query.nombreAgraviado === undefined) ? "" : req.query.nombreAgraviado;
			var apellidoAgraviado = (req.query.apellidoAgraviado === undefined) ? "" : req.query.apellidoAgraviado;
			var dniAgraviado = (req.query.dniAgraviado === undefined) ? "" : req.query.dniAgraviado;
			var codAgraviado = (req.query.codAgraviado === undefined) ? "" : req.query.codAgraviado;
			var WHERE_AGRAVIADO = "where"; // sentencia where del agraviado
			if (nombreAgraviado != "") {
				WHERE_AGRAVIADO = WHERE_AGRAVIADO + " pag.nombres like ?";
			...
			if (codAgraviado != "") {
				...
				WHERE_AGRAVIADO = WHERE_AGRAVIADO + " ag.codAgraviado='" + codAgraviado + "'";
			}
	...
	if (tipoBusqueda == "A" || tipoBusqueda == "G") {
		query = queryAgraviado;
	} else {
...
exports.guardarNotificacion = function (req, res, funcionName) {
	...
	var $descripcionBreve = req.body.descripcionBreve;
	var $medio = req.body.medio;
	...
	for (var $i = 0; $i < $cantidadNotificaciones; $i++) {
		var $Persona_tipo = $destinatarios[$i].responsable.split(";");
		var $idPersona = $Persona_tipo[0];
		var $tipoPersona = $Persona_tipo[1];
		if ($i > 0) {
			$sqlValues = $sqlValues + " , ";
		}
		var contenidoNotificacion = $destinatarios[$i].notificacion;

		$sqlValues = $sqlValues + " ('" + $codEvento + "', CURRENT_TIMESTAMP, '" + $idPersona + "', '" + $tipoPersona + "', '" + $idMotivo + "', '" + $descripcionBreve + "', '" + $medio + "', '" + $fechaEmision + "', '" + contenidoNotificacion + "') ";
	}
	var query = "INSERT into Notificacion(codEvento, fechaGeneracion, idPersona, tipoPersona, idMotivo, descripcionBreve, medio, fechaEmision, contenidoNotificacion) values " + $sqlValues + ";";
	ejecutarQUERY_MYSQL(query, [], res, funcionName, function (res, resultados) {
		...
		ejecutarQUERY_MYSQL(queryCambiaEstadoEvento, [], res, funcionName, function (res, resultados) {
			var querySelect = "SELECT idNotificacion, idPersona from Notificacion order by idNotificacion desc limit " + $cantidadNotificaciones;
...
exports.insertarMotivo = function (req, res, funcionName) {
	...
	var query = "call sp_insertarMotivo(?,@0)";
...
exports.actualizarMotivo = function (req, res, funcionName) {
	...
	var descripcion = req.query.descripcion;
	var query = "call sp_actualizarMotivo(?,?,@0)";
...
exports.getTareas = function (req, res, funcionName) {
	...
	if (fechaDesde != "" || fechaHasta != "") {
		if (fechaDesde != "" && fechaHasta != "") {
			fechaHasta = fechaHasta + " 23:59:59";
			queryWhere = validarWhere("( s.fechaProgramada between '" + fechaDesde + "' and '" + fechaHasta + "' )");
		} else {
			if (fechaDesde != "") {
				queryWhere = validarWhere("s.fechaProgramada>='" + fechaDesde + "'");
			}
			if (fechaHasta != "") {
				fechaHasta = fechaHasta + " 23:59:59";
	...
	var cantPaginas = req.query.cantPaginas;
	var registrosxpagina = req.query.registrosxpagina;
	var query = "select s.idTarea, DATE_FORMAT (s.fechaProgramada, '%Y-%m-%d %T') as fechaProgramada, s.estado, s.descripcionTarea, s.medio, s.idNotificacion, n.codEvento, s.resultado " +
...
exports.guardarResultado = function (req, res, funcionName) {
	...
	var idNotificacion = req.query.idNotificacion;
	var query = "call sp_guardarResultado(?,?,?,?,@0)";
...
exports.getGastosByCodEvento = function (req, res, funcionName) { // FALTA
	...
	if ($filtroFecha != undefined) {
		...
		$queryAdicional1 = " and g.fecha>'" + $filtroFecha + "'";
		$queryAdicional2 = " and g.fecha>'" + $filtroFecha + "'";
	}
	var $query = "select * from ((select concat('(Carta) ',g.nroCarta) as numero, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, " +
		" g.monto from CartaGarantia g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto " +
		" inner join Agraviado a on g.codAgraviado=a.codAgraviado " +
		" inner join Persona p on a.idPersona=p.idPersona " +
		" where g.codEvento='" + $codEvento + "'" + $queryAdicional1 + " and g.estado='P') UNION ALL  (select concat('N° IPRESS ', g.nroOrdenPagoAgraviado) as numero, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fechaRegistro, '%d/%m/%Y') as fechaDoc, " +
		" g.monto from OrdenPagoAgraviado g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto " +
		" inner join Agraviado a on g.codAgraviado=a.codAgraviado " +
		" inner join Persona p on a.idPersona=p.idPersona " +
		" where g.codEvento='" + $codEvento + "'" + $queryAdicional2 + " and g.estado in ( 'B', 'P' )) UNION ALL (select concat('N° IPRESS ', g.nroDocumento) as numero, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fechaEmision, '%d/%m/%Y') as fechaDoc, " +
		" g.monto from DocumentoPagoProv g inner join OrdenPagoProv op on g.nroOrdenPago = op.nroOrdenPago " +
		" inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto " +
		" inner join Agraviado a on g.codAgraviado=a.codAgraviado " +
		" inner join Persona p on a.idPersona=p.idPersona " +
		" where op.estado in ('B', 'P') and g.codEvento='" + $codEvento + "'" + $queryAdicional + ")) as v order by v.tipoGasto, STR_TO_DATE(v.fechaDoc, '%d/%m/%Y') ";
...
exports.cancelarAcuerdo = function (req, res, funcionName) {
	...
	var query = "call sp_cancelarAcuerdo(?,@0)";
...
exports.guardarAcuerdo = function (req, res, funcionName) { // FALTCOMP
	...
	var cuotaInicial = req.query.idCuotaInicial;
	var valorCuota = req.query.idValorCuota;
	var nroCuotas = req.query.idNroCuotas;
	if (cuotaInicial != "" && cuotaInicial != "0") {
		nroCuotas = parseInt(nroCuotas) + 1;
	}
				...
				if (resultados.length == 0) {
					...
					ejecutarQUERY_MYSQL(queyInsertPersona, arrayInsertPersona, res, funcionName, function (res2, resultados2) {
						...
						var queryInsertAcuerdo = "INSERT INTO Acuerdo(fechaAcuerdo, codEvento, estado, gastosAccidente, gastosAdministrativos," +
							"deudaAcordada, cuotaInicial, valorCuota, nroCuotas, fechaInicioCuotas, PeriodoCuotas, idAcuerdoAnterior, idPersonaResponsable1, idPersonaResponsable2, idPersonaResponsable3, idPersonaResponsableFinal) " +
							"select CURRENT_TIMESTAMP, '" + codEvento + "', '" + estadoAcuerdo + "', '" + gastosAccidente + "', '" + gastosAdministrativos + "', '" + deudaAcordada + "', '" + cuotaInicial + "', '" + valorCuota + "', " +
						...
						ejecutarQUERY_MYSQL(queryInsertAcuerdo, [], res, funcionName, function (res, resultados) {
							...
							if (estadoAcuerdo == "P") { // Si el estado del acuerdo que se esta registrado es pendiente se generará un cronograma
								...
								var cuotaIndex = 1;
								if (cuotaInicial != "" && cuotaInicial != "0") {
									// Inserta cuota inicial dentro del cronograma de pagos
									var queryCuotaInicial = "INSERT INTO Cronograma(valorCuota, fechaApagar, nroCuota, codEvento, idAcuerdo) " +
										"select '" + cuotaInicial + "', CURRENT_TIMESTAMP, '" + cuotaIndex + "', '" + codEvento + "', '" + idAcuerdoIngresado + "'";
									ejecutarQUERY_MYSQL(queryCuotaInicial, [], res, funcionName, "false");
									cuotaIndex++;
								}
								...
								var queryInsertGastoAdministrativos = "INSERT INTO Cronograma(valorCuota, fechaApagar, nroCuota, codEvento, idAcuerdo) " +
									"select '" + valorCuota + "', '" + fechaInicio + "', '" + cuotaIndex + "', '" + codEvento + "', '" + idAcuerdoIngresado + "'";
...
*/
exports.getGastosXperiodo = function (req, res, funcionName) {
	var $fechaInicio = req.query.fechaInicio;
	var $fechaFin = req.query.fechaFin;
	var $queryWhere;
	...
	var $query = "select codEvento from Evento where esRecupero='S' and estado in ('P', 'N') and " + $queryWhere + " order by fechaAccidente desc";
	ejecutarQUERY_MYSQL($query, [], res, funcionName, function (res, resultados) {
		var codigos = "";
		...
		if (codigosConComillas != "") {
			...
			var $queryGastosXcodEvento = "select e.codEvento, ta.descripcion as descripcionEvento, DATE_FORMAT (e.fechaAccidente, '%Y-%m-%d %T') as fechaAccidente, p.tipoPersona, d.nombre as distritoAsociado, p.calle, p.nro, p.mzLote, p.sector, p.referencia, concat(pc.nombres,' ',pc.apellidoPaterno,' ',pc.apellidoMaterno) as nombreChofer, pc.nroDocumento as dniChofer, ca.descripcion as causal1, ca2.descripcion as causal2, ch.licenciaChofer, " +
				"(concat(p.nombres,', ',p.apellidoPaterno,' ',p.apellidoMaterno)) as nombreAsociado, p.razonSocial, e.nroCAT, c.placa, (select if(sum(car.monto) is null,0,sum(car.monto)) as monto from CartaGarantia car where car.codEvento = e.codEvento and car.estado='P') + (select if(sum(ord.monto) is null,0,sum(ord.monto)) as monto from OrdenPagoAgraviado ord where ord.codEvento = e.codEvento and ord.estado in ( 'B', 'P' )) + (select if(sum(vg.monto) is null, 0, sum(vg.monto)) as monto from (select distinct gas.nroDocumento, gas.monto, gas.codEvento from DocumentoPagoProv gas inner join OrdenPagoProv op on gas.nroOrdenPago = op.nroOrdenPago where op.estado in ('B', 'P') ) as vg where vg.codEvento=e.codEvento) as montoTotalGasto, " +
				"(select date_format(gx.fechaEmision, '%Y-%m-%d %T') as fechaDoc from DocumentoPagoProv gx inner join OrdenPagoProv opr on gx.nroOrdenPago = opr.nroOrdenPago where opr.estado in ('B', 'P') and gx.codEvento=e.codEvento order by gx.fechaEmision desc limit 1) as ultimaFecha from Evento e " +
				...
				"inner join Persona p on a.idPersona=p.idPersona " +
				"inner join Cat c on e.nroCAT=c.nroCAT " +
				"left join Causal ca on i.causal1=ca.codCausal " +
				"left join Causal ca2 on i.causal2=ca2.codCausal " +
...
exports.buscarAcuerdosConOrdenesPago = function (req, res, funcionName) { // FALTCOMP
	...
	var query = "Select A.idAcuerdo, A.codEvento, DATE_FORMAT (A.fechaAcuerdo, '%d/%m/%Y') as fechaAcuerdo, A.deudaAcordada, A.cuotaInicial, " +
...
exports.buscarAcuerdos = function (req, res, funcionName) { // FALTCOMP
	...
	var query = "Select A.idAcuerdo, A.codEvento, date_format( A.fechaAcuerdo, '%d/%m/%Y') as fechaAcuerdo, A.gastosAdministrativos, A.deudaAcordada, A.cuotaInicial, " +
		"date_format(A.fechaInicioCuotas, '%d/%m/%Y') as fechaInicioCuotas, date_format(E.fechaAccidente, '%d/%m/%Y') as fechaAccidente, TP.descripcion, concat(P.nombres,' ',P.apellidoPaterno,' ',P.apellidoMaterno) as nombreAsociado, " +
...
exports.getCuotasByAcuerdo = function (req, res, funcionName) { // FALTCOMP
	...
	var query = "select c.idCuota, c.nroCuota, c.estadoCuota, DATE_FORMAT (c.fechaApagar, '%d/%m/%Y') as fechaVencimiento, c.valorCuota, c.pagosACuenta as montoPagado , DATE_FORMAT (c.fechaPago, '%d/%m/%Y') as ultimaFechaPago from Cronograma c " +
		"where c.idAcuerdo='" + $idAcuerdo + "' " + $condicionWhere + " group by c.idCuota, c.nroCuota, c.estadoCuota, c.fechaApagar, c.valorCuota, c.fechaPago, c.pagosACuenta order by c.nroCuota asc";
...
exports.efectuarPagos = function (req, res, funcionName) { // FALTCOMP
	...
	for (var $i = 0; $i < $idPagos.length; $i++) {
		...
		var queryUpdateCronograma = "Update Cronograma set pagosACuenta = pagosACuenta+" + $monto + ", fechaPago=CURRENT_TIMESTAMP, idPersonaPago='" + $idPersonaPagador + "',  estadoCuota = (case when pagosACuenta < valorCuota  then 'S' else 'P' end ) where idCuota='" + $idCuota + "'";
...
exports.eliminarUsuario = function (req, res, funcionName) {
	var idUsuario = req.query.idUsuario;
	var query = "call sp_eliminarUsuario(?,@0)";
	var arrayParametros = [idUsuario];
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName, function (res, resultados) {
		var filasAfectadas = [resultados[0].filasAfectadas];
		enviarResponse(res, filasAfectadas);
...
exports.registrarAccionUsuario = function (req, res, funcionName) {
	var idUsuario = req.query.idUsuario;
	var descripcion = req.query.descripcion;
	var query = "call sp_registrarAccionUsuario(?,?,@0)";
...
exports.insertarPerfil = function (req, res, funcionName) {
	...
	var query = "call sp_insertarPerfil(?,@0)";
	...
	if ($idPersonaTramitante == 0) { // Inserta Persona			
		...
		if ($nombres != "") { // solo registrara si el campo nombres es diferente de vacio
			...
			ejecutarQUERY_MYSQL($sqlInsertPersona, [], res, funcionName, function (res, resultados) {
				...
				$sql = "Insert into Expediente (fechaIngreso, codEvento, codAgraviado, idExpedientePrevio, nroFolios, tipoExpediente, diasRespuesta, nroDocReferencia, idPersonaQPresenta, Observaciones, idInstitucion, idProveedor, estado) " +
					"select '" + $fechaIngreso + "', '" + $codEvento + "', '" + $idAgraviado + "', '" + $idExpedientePrevio + "', '" + $nroFolios + "', '" + $idTipoExpediente + "', '" + $diasRespuesta + "', '" + $nroDocRef + "', '" + $idPersonaTramitante + "', '" + $observaciones + "', '" + $idInstitucion + "', '" + idProveedor + "', '" + estadoExpediente + "'";
				ejecutarQUERY_MYSQL($sql, [], res, funcionName, function (res, resultados) {
...
exports.getTramitesByidUsuario = function (req, res, funcionName) {
	...
	var tipoExp = req.query.tipoExp
	var query;
	var arrayParametros;
	if (tipoExp == '') {
		query = "call sp_getTramitesByidUsuario(?,?)";
		arrayParametros = [idUsuario, idAreaUsuario];
	} else { // contiene filtro de tipo de accidente
		query = "call sp_getTramitesByTipoExp_idUsuario(?,?,?)";
		arrayParametros = [idUsuario, idAreaUsuario, tipoExp];
	}
	ejecutarQUERY_MYSQL(query, arrayParametros, res, funcionName);
}

exports.uploadInformePDF = function (req, res, funcionName) {
	// create an incoming form object
	var form = new formidable.IncomingForm();
	// store all uploads in the /uploads directory
	form.uploadDir = path.join(__dirname, '../www/uploads_informe');
...
exports.confirmarTramite = function (req, res, funcionName) {
	...
	var query = "update Historial h inner join Expediente e on h.idExpediente = e.idExpediente set h.estadoNotificacion='2', h.idUsuarioDestino='" + idUsuario + "', h.fechaSalida=STR_TO_DATE('" + fechaIngreso + "', '%Y-%m-%d %T.%f'), h.finHistorial=(if(e.estado!='1', '1', '0')) where h.idHistorial in (" + idHistorial + ")";

	ejecutarQUERY_MYSQL(query, [], res, funcionName, function (res, resultados) {
		var filasAfectadas = [resultados.affectedRows];
		var estadoExpediente = req.query.estadoExpediente;
		var estadoExpedienteArray = estadoExpediente.split(",");

		var idHistorial = req.query.idHistorial
		var idHistorialArray = idHistorial.split(",");

		var idExpediente = req.query.idExpediente;
		var idExpedienteArray = idExpediente.split(",");
		...
		// Ingresa los siguientes historiales en el caso que el expediente sigue en Proceso.		
		var queryInsert = "Insert into Historial (idExpediente, idArea, idUsuario, fechaIngreso, estadoNotificacion) values ";
		var values = "";

		for (var i = 0; i < idHistorialArray.length; i++) {
			if (estadoExpedienteArray[i] == '1') {
				if (values != "") {
					values = values + " ,";
				}
				values = values + " (" + idExpedienteArray[i] + ", " + idArea + ", " + idUsuario + ", STR_TO_DATE('" + fechaIngreso + "', '%Y-%m-%d %T.%f'), '0')";
			}
		}
		if (values != "") {
			var queryGeneral = queryInsert + values;
			ejecutarQUERY_MYSQL_Extra(filasAfectadas, queryGeneral, [], res, funcionName, function (res, resultados, rows) {
				var filasAfectadas = [parseInt(resultados.affectedRows) + parseInt(rows[0])];
				enviarResponse(res, filasAfectadas);
			});
		} else {
			enviarResponse(res, filasAfectadas)
...
exports.getExpedientes = function (req, res, funcionName) {
	...
	$queryWhere = $queryWhere + " and e.estado !='A'"; //13/NOV/19 ANULAR Expedientes
	var $query = "Select e.idExpediente, e.tipoExpediente, e.estado, date_format (e.fechaIngreso, '%d/%m/%Y') as fechaExpediente, e.diasRespuesta, e.codAgraviado, e.nroFolios, nroDocReferencia, e.idInstitucion, e.idProveedor, e.Observaciones, e.idExpedientePrevio, pq.idPersona as idPersonaTramitador, concat(pq.nombres,' ',pq.apellidoPaterno,' ',pq.apellidoMaterno) as personaQpresenta, pq.nombres as nombresTramitador, CONCAT(pq.apellidoPaterno,' ',pq.apellidoMaterno) as apellidosTramitador, pq.nroDocumento, pq.telefonoMovil, pq.email, pq.calle as direccion, " +
		"ev.codEvento, ev.nroCAT, c.placa, DATE_FORMAT (ev.fechaAccidente, '%d/%m/%Y') as fechaAccidente, concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAsociado, pa.tipoPersona, pa.razonSocial, concat(pg.nombres,' ',pg.apellidoPaterno,' ',pg.apellidoMaterno) as nombresAgraviado, " +
		" pro.idProveedor from Expediente e " +
		...
		"left join Persona pq on e.idPersonaQPresenta=pq.idPersona " +
		"left join Proveedor pro on e.idProveedor=pro.idProveedor " + $queryWhere;
...
exports.getcitas = function (req, res, funcionName) { // FALTCOMP
	...
	var $query = "Select c.idCita, DATE_FORMAT (c.fechaHora, '%d/%m/%Y %T') fechaCita, c.estado, concat(pag.nombres,' ',pag.apellidoPaterno,' ',pag.apellidoMaterno) as nombresAgraviado, " +
		"c.idExpediente, e.tipoExpediente, ev.nroCAT, e.codEvento from Cita c " +
...
exports.mantenimientoCita = function (req, res, funcionName) {
	...
	var estado = req.query.estado;
	var comentario = req.query.comentario;
	var query = "call sp_mantenimientoCita(?,?,?,@0)";
...
exports.actualizarInforme = function (req, res, funcionName) {
	...
	var query = "call sp_actualizarInforme(?,?,@0)";
...
exports.actualizarPlantilla = function (req, res, funcionName) {
	...
	var query = "call sp_actualizarPlantilla(?,?,@0)";
...
exports.actualizarVariablesGlobales = function (req, res, funcionName) {
	...
	var query = "call sp_actualizarVariablesGlobales(?,?,?,?,?,?,?,?,?,?,?,?,?,@0)";
...
exports.actualizarExpediente = function (req, res, funcionName) { // FALTCOMP
	...
	ejecutarQUERY_MYSQL($query, [], res, funcionName, function (res, resultados) {
		...
		var $estadoExpediente = req.query.estadoExpediente;
		var tipoExpediente = req.query.tipoExpediente;
		var Observaciones = req.query.Observaciones;

		var subQueryExtra = ""
		if (tipoExpediente == '10') { // PAGO A IPRESS
			var idProveedor = req.query.idProveedor
			subQueryExtra = ", idProveedor = '" + idProveedor + "' "
		}

		var $query2 = "Update Expediente set estado='" + $estadoExpediente + "', fechaIngreso='" + $idFechaIngreso +
			"', Observaciones='" + Observaciones + "', tipoExpediente='" + tipoExpediente + "' " + subQueryExtra + " where idExpediente='" + $idExpediente + "'";
...
//ANULAR Expediente
exports.anularExpediente = function (req, res, funcionName) {
	var idExpediente = req.query.idExpediente;
	var queryActualizaExp = "update Expediente set estado='A' where idExpediente=" + idExpediente;
	ejecutarQUERY_MYSQL(queryActualizaExp, [], res, funcionName, "false");
...
exports.getExpedientesCriticos = function (req, res, funcionName) { // FALTCOMP
	...
	var query = "Select e.idExpediente, CONCAT(DATE_FORMAT (e.fechaIngreso, '%d/%m/%Y'),' ',DATE_FORMAT (e.fechaIngreso, '%T')) as fechaIngreso, h.estadoNotificacion, " +
		"Aorigen.Nombre as areaOrigen, Adestino.Nombre as areaDestino, CONCAT(uo.Nombres,' ',uo.Apellidos) as usuarioOrigen, CONCAT(ud.Nombres,' ',ud.Apellidos) as usuarioDestino, e.diasRespuesta, e.estado " +
		"from Expediente e " +
		...
		"where CURRENT_TIMESTAMP >= DATE_SUB(DATE_ADD(h.fechaIngreso, INTERVAL e.diasRespuesta DAY), INTERVAL " + tiempoCritico + " DAY) " +
		"and e.estado='1'" + whereCondicion;
	ejecutarQUERY_MYSQL(query, [], res, funcionName);
}
exports.eliminarArea = function (req, res, funcionName) {
	var $idArea = req.query.idArea;
	var $query = "Call sp_eliminarArea(?,@0)";
...
exports.eventoPDF = function (req, res, funcionName) {
	...
	ejecutarQUERY_MYSQL($queryAgraviados, [$codEvento], res, funcionName, function (res, resultados) {
		...
		var $queryGastos = "Select v.idTipoGasto, sum(v.montoGasto) as montoGasto, v.descripcion from ((select g.idEtapa as idTipoGasto, sum(g.monto) as montoGasto, t.descripcion from Gasto g " +
			" inner join TipoGasto t on g.idEtapa=t.idTipoGasto where g.codEvento=? group by g.idEtapa, t.descripcion) UNION ALL " +
			"(select c.idEtapa as idTipoGasto, sum(c.monto) as montoGasto, t.descripcion from CartaGarantia c " +
			" inner join TipoGasto t on c.idEtapa=t.idTipoGasto where c.codEvento=? and c.estado='P' group by c.idEtapa, t.descripcion) UNION ALL " +
			"(select o.idEtapa as idTipoGasto, sum(o.monto) as montoGasto, t.descripcion from OrdenPago o " +
			" inner join TipoGasto t on o.idEtapa=t.idTipoGasto where o.codEvento=? and o.estado in ( 'B', 'P' ) group by o.idEtapa, t.descripcion)) as v group by v.idTipoGasto, v.descripcion";

		ejecutarQUERY_MYSQL($queryGastos, [$codEvento, $codEvento, $codEvento], res, funcionName, function (res, resultados) {
			for (var i = 0; i < resultados.length; i++) {
			...
			$filasGastos = $filasGastos +
				...
				"</tr>";
			if ($filasGastos != "") {
				$tablaGastos = "<br><br><table style='width:350px; font-size:14px; margin-left:20px;'>" +
					"<col style='width:50%'>" +
					"<col style='width:50%'>" +
					$filasGastos + "</table>";
			}
			...
			var $tablaCausales = "";
			if ($trsCausales != "") {
				$tablaCausales = "<br><br><table style='width:850px; font-size:14px; margin-left:20px;'>" +
					"<col style='width:5%'>" +
					"<col style='width:90%'>" +
					$trsCausales +
					"</table>";
			...
			// Obtiene las plantilla pie de pagina y el contenido de la notificacion
			var queryPlantillas = "Select contenidoNotificacion, (select c.plantillaPieNotificacion from ConstantesGenerales c limit 1) as footer from Notificacion where idNotificacion = ? ";
			ejecutarQUERY_MYSQL(queryPlantillas, [$idNotificacion], res, funcionName, function (res, resultados) {

				var pie = resultados[0].footer;
				var contenidoNotificacion = resultados[0].contenidoNotificacion
				...
				var body =
					...
					"</head>" +
					"<body style='padding-top:14px;zoom: 75%;'>" +
					...
					"<u>AVISO DE ACCIDENTE DE TRANSITO</u>" +
					"</div>" +
...
exports.docPDF = function (req, res, funcionName) {
	...
	var $cabecera3 = "<hr>" +
		...
		"<col style='width:10%'>" +
		"<col style='width:11%'>" +
		"<tr style='font-size:14px; height:30px; vertical-align:middle;'>" +
	...
	switch ($numModulo) {
		case "1":
			...
			var query = "select c.nroCuota, DATE_FORMAT(c.fechaApagar, '%d/%m/%Y') as fechaVencimiento, c.valorCuota, c.pagosACuenta as montoPagado  from Cronograma c " +
				"where c.idAcuerdo=? and c.estadoCuota in ('D', 'S', 'G') order by c.fechaApagar, c.nroCuota";
			ejecutarQUERY_MYSQL(query, [$idAcuerdo], res, funcionName, function (res, resultados) {
				//("encuentra las cuotas");
				for (var i = 0; i < resultados.length; i++) {
					$saldo = resultados[i].valorCuota - resultados[i].montoPagado;
					$totalCronograma = $totalCronograma + $saldo;
					$filasTabla = $filasTabla + "<tr style='background-color:#FAFAFA; font-size:14px; height:30px; vertical-align:middle;'>" +
				...
				$parteUltima = "<!-- RESULTADO FINAL -->" +
					...
					"<col style='width:30%;'>" +
					"<col style='width:30%;'>" +
					"<tr style='background-color:#4485A6; color:white; font-size:14px; height:32px; vertical-align:middle;'>" +
					...
					"<col style='width:70%;'>" +
					"<col style='width:30%;'>" +
					"<tr style='font-size:14px; font-weight:bold; height:30px; vertical-align:middle; border-width:1px; border-style:solid; border-color:gray;'>" +
		...
		case "2":
			...
			var query = "select c.nroCuota, c.estadoCuota, DATE_FORMAT (c.fechaApagar, '%d/%m/%Y') as fechaVencimiento, c.valorCuota, c.pagosACuenta as montoPagado, DATE_FORMAT (c.fechaPago, '%d/%m/%Y') as ultimaFechaPago " +
				"from Cronograma c where c.idAcuerdo=? order by c.fechaApagar, c.nroCuota";
			ejecutarQUERY_MYSQL(query, [$idAcuerdo], res, funcionName, function (res, resultados) {
				...
				$parteUltima = "<!-- RESULTADO FINAL -->" +
					"<br>" +
					"<table style='margin:auto; width:850px; border-width:1px; border-style:solid; border-color:gray;'>" +
					"<col style='width:8%;'>" +
					"<col style='width:20%;'>" +
					"<col style='width:13%;'>" +
					"<col style='width:20%;'>" +
			...
			for (var i = 0; i < evento.length; i++) {
				...
				ejecutarQUERY_MYSQL_Extra(evento[i], verificaCodEvento, [], res, funcionName, function (res, resultados, evento) {
					...
					if (contadorEventos == 0) {
						...
						//("el evento "+evento.codEvento+" ya se encuentra registrado")
...
exports.informeExpediente = function (req, res, funcionName) {
	...
	var $ancho = "780px";
	var direcionLocal = "Av. Brasil 1124 - Pueblo Libre";
	var $body =
		...
		"<p style='text-align:justify; font-size:14px; line-height:18.5px; padding-top:7px;'>" +
	...
	var $html =
		...
		"<meta content='text/html; charset=utf-8' http-equiv='Content-Type'>" +
		"</head>" +
		"<body style='padding-top:0px; background-color:;zoom: 75%;'>" +
		"<br>" +
...
exports.getLiquidacion = function (req, res, funcionName) {
	...
	var condicion = "";
	switch (tipoBusqueda) {
		case 'E': // Numero evento
			condicion = " e.codEvento='" + codigo + "' ";
			break;
		case 'P': // numero placa
			condicion = " c.placa= '" + codigo + "' ";
			break;
		case 'C': // numero cat
			condicion = " e.nroCAT = '" + codigo + "' ";
			break;
		default:
			condicion = " e.codEvento='" + codigo + "' ";
			break;
	...
	var query = " SELECT e.codEvento as numcentral, c.placa, e.nroCAT as cat, a.codAgraviado as numsiniestro, " +
		" date_format(e.fechaRegistro, '%d/%m/%Y') as fechaevento , concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno)  as nombre, if(pa.tipoPersona='J', pa.razonSocial, concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno)) as nombreasegurado, (select sum(car.monto) as monto from CartaGarantia car where car.codAgraviado = a.codAgraviado and car.estado='P') + (select if(sum(ord.monto) is null,0,sum(ord.monto)) as monto from OrdenPagoAgraviado ord where ord.codAgraviado = a.codAgraviado and ord.estado in ( 'B', 'P' )) + (select if(sum(gas.monto) is null,0,sum(gas.monto)) as monto from (Select distinct gs.codEvento, gs.monto, gs.nroDocumento, gs.codAgraviado from DocumentoPagoProv gs inner join OrdenPagoProv op on gs.nroOrdenPago = op.nroOrdenPago where op.estado in ('B', 'P')) as gas where gas.codAgraviado = a.codAgraviado) as Total, date_format(e.fechaAccidente, '%d/%m/%Y') as fechasiniestro FROM Evento e " +
		"INNER JOIN Agraviado a on e.codEvento = a.codEvento " +
		"INNER JOIN Persona p on a.idPersona = p.idPersona " +
		"INNER JOIN Cat c on e.nroCAT = c.nroCAT " +
		"INNER JOIN Asociado aso on c.idAsociado = aso.idAsociado " +
		"INNER JOIN Persona pa on aso.idPersona = pa.idPersona " +
		"where " + condicion + " ORDER BY e.codEvento desc, a.codAgraviado";
...
exports.liquidacionPDF = function (req, res, funcionName) {
	...
	var $cabecera = "<table style=' margin:auto; width:640px; font-size:15px; margin-top:-10px;'>" +
		"<col style='width:15%;'>" +
		"<col style='width:12%;'>" +
		"<col style='width:45%;'>" +
		"<col style='width:15%;'>" +
		"<col style='width:20%;'>" +
		"<tr style='background-color:#A7C942; color: white; font-weight:bold; font-size:16px;'>" +
		"<td>Factura/Orden</td>" +
		"<td>Fecha</td>" +
	...
	var $query = "select * from ((select g.nroCarta as numero, if(ppro.tipoPersona='J', ppro.razonSocial, concat(ppro.nombres,' ',ppro.apellidoPaterno,' ', ppro.apellidoMaterno)) as proveedor, 'CARTA GARANTIA' as comprobante, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, " +
		" g.monto from CartaGarantia g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto " +
		" inner join Agraviado a on g.codAgraviado=a.codAgraviado " +
		" inner join Persona p on a.idPersona=p.idPersona " +
		" left join Proveedor n on g.idProveedor = n.idProveedor " +
		" left join Persona ppro on n.idPersona = ppro.idPersona " +
		" where g.codAgraviado='" + $numsiniestro + "' and g.estado='P') UNION ALL  (select g.nroOrdenPagoAgraviado as numero, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as proveedor, /*t.descripcion*/ 'NO ESPECIFICADO' as comprobante, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fechaRegistro, '%d/%m/%Y') as fechaDoc, " +
		" g.monto from OrdenPagoAgraviado g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto " +
		" inner join Agraviado a on g.codAgraviado=a.codAgraviado " +
		" inner join Persona p on a.idPersona=p.idPersona " +
		" /*left join TipoDocumento t on g.idTipoDocumento = t.idTipoDocumento*/" +
		" where g.codAgraviado='" + $numsiniestro + "' and g.estado in ( 'B', 'P' )) UNION ALL (select g.nroDocumento as numero, if(ppro.tipoPersona='J', ppro.razonSocial, concat(ppro.nombres,' ',ppro.apellidoPaterno,' ', ppro.apellidoMaterno)) as proveedor, t.descripcion as comprobante, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fechaEmision, '%d/%m/%Y') as fechaDoc, " +
		" g.monto from DocumentoPagoProv g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto " +
		" inner join Agraviado a on g.codAgraviado=a.codAgraviado " +
		" inner join Persona p on a.idPersona=p.idPersona " +
		" left join Proveedor prov on g.idProveedor = prov.idProveedor " +
		" left join Persona ppro on prov.idPersona = ppro.idPersona " +
		" left join TipoDocumento t on g.idTipoDoc = t.idTipoDocumento " +
		" inner join OrdenPagoProv op on op.nroOrdenPago=g.nroOrdenPago " +
		" where g.codAgraviado='" + $numsiniestro + "' and op.estado!='I' )) as v order by v.tipoGasto, STR_TO_DATE(v.fechaDoc, '%d/%m/%Y')";
	//Debe incluir CG en estado 'P' y facturas de una Orden Apro(B)ada o (P)agada (NO Ingresada) sino se duplican
	ejecutarQUERY_MYSQL($query, [], res, funcionName, function (res, resultados) {
		var $tablasDetalle = "";
		var $cont = 0;
		var $totalGlobal = 0;
		...
		for ($cont; $cont < resultados.length; $cont++) {
			if ($cont == 0) { // La primera vez
				$tablasDetalle = $tablasDetalle +
					"<div style='font-size:21px; font-weight:bold; margin-left:14px;'>" + resultados[$cont]['tipoGasto'] + "</div>" + $cabecera +
					"<br>" +
					"<tr>" +
				...
				if (resultados[$cont]['tipoGasto'] != $descripcionAnterior) {
					$tablasDetalle += "<tr>" +
						...
						"</tr>" +
						"</table><br>" +
						"<div style='font-size:21px; font-weight:bold; margin-left:14px;'>" + resultados[$cont]['tipoGasto'] + "</div><br>" + $cabecera;
					$totalTipo = 0;
				}
				$tablasDetalle += "<tr>" +
					"<td>" + resultados[$cont]['numero'] + "</td>" +
		...
		var body =
			...
			"<tr style='color:gray; font-size:15px;'>" +
			...
			"<br>" +
			"<table style='width:640px; font-size:16px; margin:auto;'>" +
			'<col style="width:13%;">' +
			'<col style="width:16%;">' +
			'<col style="width:18%;">' +
			'<col style="width:33%;">' +
			...
			'<col style="width:14%;">' +
			"<tr>" +
			"<td colspan='6' style='font-size:21px;'>" +
			...
			"<br>" +
			"<br>" +
			"<label style='font-weight:bold; font-size:15px;'>TOTAL DE GASTOS REALIZADOS A LA FECHA:  S/. " + $totalGlobal + "</label>" +
			"<br>" +
			"<label style='font-size:15px;'>*** Estos Gastos se pueden incrementar, conforme se vaya generando en su futuro, conforme a las normas legales ***</label>" +
			"</div>" +
			"</body>" +
			"</html>";
...
exports.consultarSolicitud = function (req, res, funcionName) {
	var idExpediente = req.query.idExpediente;
	var nroCAT = req.query.nroCAT;
	var $query = "Select e.idExpediente, e.tipoExpediente, e.estado, date_format (e.fechaIngreso, '%d/%m/%Y') as fechaExpediente, e.diasRespuesta, e.codAgraviado, e.nroFolios, nroDocReferencia, e.idInstitucion, e.Observaciones, e.idExpedientePrevio, pq.idPersona as idPersonaTramitador, concat(pq.nombres,' ',pq.apellidoPaterno,' ',pq.apellidoMaterno) as personaQpresenta, pq.nombres as nombresTramitador, CONCAT(pq.apellidoPaterno,' ',pq.apellidoMaterno) as apellidosTramitador, pq.nroDocumento, pq.telefonoMovil, pq.email, pq.calle as direccion, " +
		"ev.codEvento, ev.nroCAT, c.placa, DATE_FORMAT (ev.fechaAccidente, '%d/%m/%Y') as fechaAccidente, concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAsociado, pa.tipoPersona, pa.razonSocial, concat(pg.nombres,' ',pg.apellidoPaterno,' ',pg.apellidoMaterno) as nombresAgraviado " +
		"from Expediente e " +
		"left join Evento ev on e.codEvento=ev.codEvento " +
		"left join Cat c on ev.nroCAT = c.nroCAT " +
		...
		"left join Persona pg on ag.idPersona = pg.idPersona " +
		"left join Persona pq on e.idPersonaQPresenta=pq.idPersona " +
		"where ev.nroCAT=? and e.idExpediente=? and e.tipoExpediente<7 and e.estado != 'A' "; //13/NOV/19 ANULAR Expedientes
...
exports.consultarTramite = function (req, res, funcionName) {
	...
	var $query = "Select e.idExpediente, e.tipoExpediente, e.estado, date_format (e.fechaIngreso, '%d/%m/%Y') as fechaExpediente, e.diasRespuesta, e.codAgraviado, e.nroFolios, nroDocReferencia, e.idInstitucion, e.Observaciones, e.idExpedientePrevio, pq.idPersona as idPersonaTramitador, concat(pq.nombres,' ',pq.apellidoPaterno,' ',pq.apellidoMaterno) as personaQpresenta, pq.nombres as nombresTramitador, CONCAT(pq.apellidoPaterno,' ',pq.apellidoMaterno) as apellidosTramitador, pq.nroDocumento, pq.telefonoMovil, pq.email, pq.calle as direccion, " +
		...
		"left join Persona pg on ag.idPersona = pg.idPersona " +
		"left join Persona pq on e.idPersonaQPresenta=pq.idPersona " +
		"where e.nroDocReferencia=? and e.idExpediente=? and e.tipoExpediente>6 and e.estado != 'A' "; //13/NOV/19 ANULAR Expedientes
...
exports.getAgraviadosXrecupero = function (req, res, funcionName) {
	...
	var $query = "select codEvento from Evento where esRecupero='S' and " + $queryWhere + " order by codEvento desc"; // obtiene eventos recupero
	ejecutarQUERY_MYSQL($query, [], res, funcionName, function (res, resultados) {
		var codigos = "";
		var codigosConComillas = "";
		for (var i = 0; i < resultados.length; i++) {
		...
		if (codigosConComillas != "") {
			//actualizarGastos(codigos, res, funcionName, function(res, resultados){
			var queryGetEventosAgraviados = " select concat (p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombreAgraviado, " +
				" concat (pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAsociado, " +
				" concat (pr1.nombres,' ',pr1.apellidoPaterno,' ',pr1.apellidoMaterno) as nombreResponsable1, " +
				" concat (pr2.nombres,' ',pr2.apellidoPaterno,' ',pr2.apellidoMaterno) as nombreResponsable2, " +
				" concat (pr3.nombres,' ',pr3.apellidoPaterno,' ',pr3.apellidoMaterno) as nombreResponsable3, " +
				" concat (prF.nombres,' ',prF.apellidoPaterno,' ',prF.apellidoMaterno) as nombreRespFinal, " +
				" ac.idPersonaResponsable1, ac.idPersonaResponsable2, ac.idPersonaResponsable3, ac.idPersonaResponsableFinal, " +
				" a.codEvento, a.codAgraviado, a.tipo , ta.descripcion as tipoAccidente, " +
				" date_format (e.fechaAccidente, '%d/%m/%Y') as fechaAccidente, e.nroCAT, c1.descripcion as causal1, " +
				" c2.descripcion as causal2, (select sum(car.monto) as monto from CartaGarantia car where car.codAgraviado = a.codAgraviado and car.estado='P') + (select if(sum(ord.monto) is null,0,sum(ord.monto)) as monto from OrdenPago ord where ord.codAgraviado = a.codAgraviado and ord.estado in ( 'B', 'P' )) + (select if(sum(gas.monto) is null,0,sum(gas.monto)) as monto from (Select distinct gs.codEvento, gs.monto, gs.nroDocumento, gs.codAgraviado from Gasto gs) as gas where gas.codAgraviado = a.codAgraviado) as importe from Evento e " +
				" left join Acuerdo ac on e.codEvento = ac.codEvento " +
				" inner join Cat ct on e.nroCAT = ct.nroCAT " +
				" inner join Asociado aso on ct.idAsociado = aso.idAsociado " +
				" inner join Persona pa on aso.idPersona = pa.idPersona " +
				" left join Persona pr1 on ac.idPersonaResponsable1 = pr1.idPersona " +
				" left join Persona pr2 on ac.idPersonaResponsable2 = pr2.idPersona " +
				" left join Persona pr3 on ac.idPersonaResponsable3 = pr3.idPersona " +
				" left join Persona prF on ac.idPersonaResponsableFinal = prF.idPersona " +
				" inner join Agraviado a on e.codEvento = a.codEvento " +
				" inner join Persona p on a.idPersona=p.idPersona " +
				" inner join Informe i on e.codEvento = i.codEvento " +
				" left join Causal c1 on i.causal1 = c1.codCausal " +
				" left join Causal c2 on i.causal2 = c2.codCausal " +
				" left join TipoAccidente ta on ta.idTipoAccidente = i.idTipoAccidente " +
				" where e.estado != 'T' and e.codEvento in (" + codigosConComillas + ") " +
				" order by e.codEvento desc ";
			ejecutarQUERY_MYSQL(queryGetEventosAgraviados, [], res, funcionName);
			//});
		} else {
			enviarResponse(res, []);
		}
...
exports.getPlantillaNotificacion = function (req, res, funcionName) {
	var query = "Select plantillaCuerpoNotificacion as plantillaNotificacion from ConstantesGenerales";
	ejecutarQUERY_MYSQL(query, [], res, funcionName);
}
exports.getInfoEventoAyuda = function (req, res, funcionName) { // Muestra la informacion para la notificacion 
	var codEvento = req.query.codEvento;
	var query = "Select e.nroCAT, c.placa, " +
		"concat('Nro Doc :',pa.nroDocumento,' - ', if(pa.tipoPersona='J', pa.razonSocial, concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno ))) as asociado, " +
		"concat('Nro Doc :',pp.nroDocumento,' - ', if(pp.tipoPersona='J', pp.razonSocial, concat(pp.nombres,' ',pp.apellidoPaterno,' ',pp.apellidoMaterno ))) as propietario, " +
		"concat('Nro Doc :',pc.nroDocumento,' - ', concat(pc.nombres,' ',pc.apellidoPaterno,' ',pc.apellidoMaterno ), ' LIC : ', ch.licenciaChofer, ' Clase : ',ch.claseChofer) as chofer, " +
		"concat('fecha: ', date_format (e.fechaAccidente, '%d/%m/%Y %H:%i'), ' Dir: ',e.lugarAccidente, ' Distrito: ',d.nombre, ' Prov: ',pro.nombreProvincia) as fechaLugar, " +
		"(Select GROUP_CONCAT(ca.descripcion SEPARATOR ', ') FROM Causal ca where ca.codCausal in (i.causal1, i.causal2)) as causales , " +
		"(select GROUP_CONCAT(concat(vz.descripcion,': ',vz.montoGasto) SEPARATOR ', ') from (Select v.descripcion, sum(v.montoGasto) as montoGasto from ((select g.idEtapa as idTipoGasto, sum(g.monto) as montoGasto, t.descripcion from Gasto g  " +
		"inner join TipoGasto t on g.idEtapa=t.idTipoGasto where g.codEvento=? group by g.idEtapa, t.descripcion) UNION ALL " +
		"(select c.idEtapa as idTipoGasto, sum(c.monto) as montoGasto, t.descripcion from CartaGarantia c " +
		" inner join TipoGasto t on c.idEtapa=t.idTipoGasto where c.codEvento=? and c.estado='P' group by c.idEtapa, t.descripcion) UNION ALL " +
		"(select o.idEtapa as idTipoGasto, sum(o.monto) as montoGasto, t.descripcion from OrdenPago o  inner join TipoGasto t on o.idEtapa=t.idTipoGasto where o.codEvento=? and o.estado in ( 'B', 'P' ) group by o.idEtapa, t.descripcion)) as v group by v.idTipoGasto, v.descripcion) vz) " +
		"as gastos, " +
		"(select group_concat(concat(ag.codAgraviado,' - ',pag.nroDocumento,' - ',concat(pag.nombres,' ',pag.apellidoPaterno,' ',pag.apellidoMaterno)) SEPARATOR '<br> ') from Agraviado ag inner join Persona pag on ag.idPersona = pag.idPersona where ag.codEvento = ?) as agraviados " +
		"from Evento e " +
		"left join Cat c on e.nroCAT = c.nroCAT " +
		"left join Asociado a on c.idAsociado = a.idAsociado " +
		"left join Persona pa on a.idPersona = pa.idPersona " +
		"left join Propietario pr on e.idPropietario = pr.idPropietario " +
		"left join Persona pp on pr.idPersona = pp.idPersona " +
		"left join Chofer ch on e.idChofer = ch.idChofer " +
		"left join Persona pc on ch.idPersona = pc.idPersona " +
		"left join Distrito d on e.idDistrito = d.idDistrito " +
		"left join Provincia pro on d.idProvincia = pro.idProvincia " +
		"left join Informe i on e.codEvento = i.codEvento " +
		"where e.codEvento = ?";

	var parametros = [codEvento, codEvento, codEvento, codEvento, codEvento];
	ejecutarQUERY_MYSQL(query, parametros, res, funcionName);
}
exports.actualizarNotificacion = function (req, res, funcionName) {
	var motivo = req.body.motivo;
	var notificacion = req.body.notificacion;
	var medio = req.body.medio;
	var idNotificacion = req.body.idNotificacion;
	var query = "Update Notificacion set idMotivo = ?, medio=?, contenidoNotificacion=? where idNotificacion=? ";
	ejecutarQUERY_MYSQL(query, [motivo, medio, notificacion, idNotificacion], res, funcionName, "affectedRows");
}
...
exports.busquedaAvanzadaTramites = function (req, res, funcionName) { // Realiza la busqueda avanzada para los movimientos de tramites documentarios
	var opciones = req.body.filtros;
	var p_idUsuario = req.body.idUsuario;
	var p_idAreaUsuario = req.body.idAreaUsuario;
	var queryWhere = new QueryWhere(" where ((h.idUsuario=" + p_idUsuario + " and h.fechaSalida is null) or (h.idAreaDestino=" + p_idAreaUsuario + " and ( (h.estadoNotificacion='1'  and h.finHistorial='0') or (h.estadoNotificacion='2'  and h.finHistorial='1') ) )) ");
	var query = "Select h.idHistorial, e.idExpediente, e.tipoExpediente, h.idArea as idAreaOrigen, Aorigen.Nombre as nombreArea, " + "h.idAreaDestino, date_format (h.fechaIngreso, '%Y-%m-%d %T') as fechaIngreso, " +
		"concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) as nombreAgraviado, e.codEvento, " +
		"concat(perasoc.nombres,' ',perasoc.apellidoPaterno,' ',perasoc.apellidoMaterno) as nombreAsociado, perasoc.tipoPersona, perasoc.razonSocial , " +
		"e.nroDocReferencia, " +
		"e.nroFolios, e.Observaciones, e.estado as estadoExpediente, h.estadoNotificacion, " +
		"CONCAT(usu.Nombres,' ',usu.Apellidos) as usuarioDestino, h.comentarios, h.idUsuarioDestino, h.idUsuario as idUsuarioOrigen, h.informe, " +
		"ti.descripcion as tipoAccidente, concat(per.nombres,' ',per.apellidoPaterno,' ',per.apellidoMaterno) as beneficiario , " +
		"nosoc.nombre as nosocomio, ins.nombre as IPRESS, date_format (e.fechaIngreso, '%d/%m/%Y') as fechaIngresoExp, a.codAgraviado, c.placa, " +
		"date_format (event.fechaAccidente, '%d/%m/%Y') as fechaAccidente " +
		"from Expediente e " +
		"left join Informe i on i.codEvento = e.codEvento " +
		"left join TipoAccidente ti on i.idTipoAccidente = ti.idTipoAccidente " +
		"left join Persona per on e.idPersonaQPresenta = per.idPersona " +
		"inner join Historial h on e.idExpediente=h.idExpediente " +
		"inner join Area Aorigen on h.idArea=Aorigen.idArea " +
		"left join Agraviado a on e.codAgraviado=a.codAgraviado " +
		"left join Persona pa on a.idPersona=pa.idPersona " +
		"left join Evento event on e.codEvento=event.codEvento " +
		"left join Cat c on event.nroCAT=c.nroCAT " +
		"left join Asociado aso on c.idAsociado=aso.idAsociado " +
		"left join Persona perasoc on aso.idPersona=perasoc.idPersona " +
		"left join Nosocomio nosoc on a.idNosocomio = nosoc.idNosocomio " +
		"left join Instituciones ins on e.idInstitucion = ins.idInstitucion " +
		"left join UsuarioIntranet usu on h.idUsuarioDestino=usu.idUsuario ";
	if (opciones["nroExpediente"] != "") {
		queryWhere.validarWhere(" e.idExpediente='" + opciones["nroExpediente"] + "' ");
	}
	if (opciones["codAgraviado"] != "") {
		queryWhere.validarWhere(" a.codAgraviado='" + opciones["codAgraviado"] + "' ");
	}
	if (opciones["evento"] != "") {
		queryWhere.validarWhere(" event.codEvento='" + opciones["evento"] + "' ");
	}
	if (opciones["placa"] != "") {
		queryWhere.validarWhere(" c.placa='" + opciones["placa"] + "' ");
	}
	if (opciones["agraviado"] != "") {
		queryWhere.validarWhere(" concat(pa.nombres,' ',pa.apellidoPaterno,' ',pa.apellidoMaterno) like '%" + opciones["agraviado"] + "%' ");
	}
	if (opciones["beneficiario"] != "") {
		queryWhere.validarWhere(" concat(per.nombres,' ',per.apellidoPaterno,' ',per.apellidoMaterno) like '%" + opciones["agraviado"] + "%' ");
	}
	queryWhere.validarWhere(" e.estado !='A' "); // 13/NOV/19 ANULAR Expedientes
	query = query + queryWhere.getQueryWhere();
	ejecutarQUERY_MYSQL(query, [], res, funcionName);
}