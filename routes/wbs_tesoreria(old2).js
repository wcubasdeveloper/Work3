...
exports.getListaContratosImpresos = function(req, res, funcionName){
    ...
    var query = "select " +
        "c.idContrato, date_format(c.fechaEmision, '%d/%m/%Y') as fechaEmision, (select cr.nroCuota from Contrato_Renovacion cr where cr.idContrato = c.idContrato order by cr.idContratoRenovacion desc limit 1) as utlCuota, " +
        "e.nombreCorto, c.nCuotas, c.flota, " +
        "date_format(c.fechaVigenciaContr, '%d/%m/%Y') as fechaIniVigencia, " +
        "date_format(DATE_ADD(c.fechaVigenciaContr, INTERVAL 1 YEAR), '%d/%m/%Y') as fechaFinVigencia " +
        "from Contrato c "+
...
exports.getListaContratosRenovacion = function(req, res, funcionName){
    ...
    if(fechaDesde!="" || fechaHasta!=""){
        if(fechaDesde!="" && fechaHasta!=""){
            queryWhere+=" and (r.fechaRenovacion between '"+fechaDesde+"' and '"+fechaHasta+"' ) ";
        }else{
            if(fechaDesde!=""){
                queryWhere += " and r.fechaRenovacion>='"+fechaDesde+"'";
            }
            if(fechaHasta!=""){
                queryWhere += " and r.fechaRenovacion<='"+fechaHasta+"'";
            }
        }
    }
    var query = "select r.idContratoRenovacion, r.estado, " +
        "c.idContrato, r.nroCuota as ultCuota , date_format(r.fechaRenovacion, '%d/%m/%Y') as fechaRenovacion, " +
        ...
        " and r.idContratoRenovacion in (select max(cr.idContratoRenovacion) from Contrato_Renovacion cr group by cr.idContrato) order by r.fechaRenovacion desc";
