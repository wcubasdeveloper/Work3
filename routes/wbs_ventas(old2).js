                        ...
                        for(var i=0; i<listaCertificados.length; i++){
                            ...
                            abstractGuardarActualizarVehiculo_contrato(res, funcionName, listaCertificados[i], function(registroCertificado){
                                ...
                                if(registroCertificado.registrado == true){
                                    ...
                                    var queryUpdateCertificado = "Update Contrato_Certificados set idContrato=?, nroOrden=?, nroCertificado=?, idVehiculo=?, valorCuota=?, nroCuota=?, precio=? where idContratoCertificado = ?";
                                    var params = [idContrato, nroOrden, nroCertificado, idVehiculo, valorCuota, nroCuota, precio, idContratoCertificado]
                                    ejecutarQUERY_MYSQL(queryUpdateCertificado, params, res, funcionName, "false");
                                }else{
                                    // registra:
                                    var queryInsertCertificado = "Insert into Contrato_Certificados (idContrato, nroOrden, nroCertificado, idVehiculo, valorCuota, nroCuota, precio) values (?,?,?,?,?,?,?)"
                                    var params = [idContrato, nroOrden, nroCertificado, idVehiculo, valorCuota, nroCuota, precio]
...
exports.imprimirCATS = function(req, res, funcionName){
    ...
    // registra el contrato de la renovacion:
    ...
    ejecutarQUERY_MYSQL(queryInsertRenovacion, parametros, res, funcionName, function(res, resultados){
        // cambia el estado del contrato a Impreso
        var idContrato = req.body.idContrato
        ...
        ejecutarQUERY_MYSQL(updateContrato, parametros, res, funcionName, function(res, resultados){
            // Registra los certificados (TABLA CERTIFICADO)
            ...
            if(certificadosInsertar!=""){
                console.log("insertando certificados .. "+certificadosInsertar)
                ...
                ejecutarQUERY_MYSQL(certificadosInsertar, [], res, funcionName, function(res, resultados){
                    ...
                    ejecutarQUERY_MYSQL(queryInsertGuia, parametros, res, funcionName, function(res, resultados){
                        ...
                        // registra los detalles de la guia
                        var certificadosList = req.body.listaFlota
                        var detalles = []
                        var inicioCertif = 0;
                        var finCertif = 0;
                        ...
                        if(queryInsertDetalles!=""){
                            ...
                            ejecutarQUERY_MYSQL(queryInsertDetalles, [], res, funcionName, function(res, resultados){
                                ...
                                // despues de guardar los detalles, la Guia de Liquidacion:
                                var idConcesionario = req.body.idConcesionario
                                var idUsuario = req.body.idUsuarioUpdate
                                var idUsuarioResp = req.body.idUsuarioUpdate
                                var ultActualizaUsuario = req.body.idUsuarioUpdate
                                ...
                                ejecutarQUERY_MYSQL(queryInsertLiquidacion, parametros, res, funcionName, function(res, resultados){
                                    ...
                                    // registra los detalles de la liquidacion
                                    ...
                                    ejecutarQUERY_MYSQL(queryInsertVentaDetalles, [], res, funcionName, function(res, resultados){
                                        ...
                                        // guarda los movimientos del certificado y actualiza el id del movimiento en la tabla Certificado
                                        var certificadosList = req.body.listaFlota;
                                        for(var i=0; i<certificadosList.length; i++){
...
function registrarCertificadoVendidoCAT(res, funcionName, registroCertificado){
    ...
    console.log("registrando los movimientos de los certificados .. ")
    var nroCertificado = registroCertificado.nCertificado
    var idArticulo = registroCertificado.idArticulo
    var tipOperacion = "E"
    var idUbicacion = registroCertificado.idConcesionario
    var idUsuarioResp = registroCertificado.idUsuarioResp
    var idGuia = registroCertificado.idGuia
    var idGuiaSalida = registroCertificado.idGuiaLiquidacion
    var estado = "V"
    var ultActualizaUsuario = registroCertificado.ultActualizaUsuario

    var queryInsertMovimiento = "Insert into Certificado_movimiento(nroCertificado, idArticulo, tipOperacion, idUbicacion,idUsuarioResp, idGuia, fechaOperacion, fechaSalida, idGuiaSalida, estado, ultActualizaUsuario, ultActualizaFecha ) " +
        "values (?,?,?,?,?,?, now(), now(), ?,?,?, now())"
    var parametros = [nroCertificado, idArticulo, tipOperacion, idUbicacion, idUsuarioResp, idGuia, idGuiaSalida, estado, ultActualizaUsuario]
    ...
    ejecutarQUERY_MYSQL_Extra(registroCertificado, queryInsertMovimiento, parametros, res, funcionName, function(res, resultados, certificado){
        var idMovimiento = resultados.insertId
        console.log("movimiento registrado : "+idMovimiento)
        var nroCertificado = certificado.nCertificado
        // actualiza el id del movimiento en la tabla de CERTIFICADO
        var queryUpdateCertif = "Update Certificado set estadoRegistroCAT = '9', ultimoMovimiento=?, ultActualizaFecha=now() where nroCertificado=? "
        var params = [idMovimiento, nroCertificado]
        ejecutarQUERY_MYSQL(queryUpdateCertif, params, res, funcionName, "false")
        // registra u obtiene el idAsociado
        registrarAsociado(res, funcionName, certificado, function(certif){
            ...
            var queryInsert="Insert into Cat (nroCAT, idAsociado, placa, marca, modelo, annoFabricacion, nMotorserie, fechaInicio, fechaCaducidad, monto, idConcesionario," +
                " fechaEmision, idVehiculo, conDeuda, fechaLiquidacion, ultActualizaUsuario, ultActualizaFecha)" +
                " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?, now(), ?, now())"

            var parametros = [nroCAT, idAsociado, placa, marca, modelo, annoFabricacion, nMotorserie, fechaInicio, fechaCaducidad, monto, idConcesionario, fechaEmision, idVehiculo, conDeuda, ultActualizaUsuario]

            ejecutarQUERY_MYSQL(queryInsert, parametros, res, funcionName, "false")
        })
    })
}
function registrarAsociado(res, funcionName, registroCertificado, callback){ // busca si existe el asociado registrado sino lo registra
...
exports.getContratoDetalle = function(req, res, funcionName){
    ...
    var query = "select idContrato, date_format(fechaEmision, '%d/%m/%Y') as fechaEmision,"+
        ...
        " e.nombreCorto as nombreEmpresa, pe.idPersona as idPersonaEmpresa, pe.tipoPersona, pe.nroDocumento as nroDocumentoPersonaEmpr,"+
        " prl.nroDocumento as DNIReprLegal"+
        " from Contrato c"+
    ...
    ejecutarQUERY_MYSQL(query, parametros, res, funcionName, function(res, resultados){
        if(resultados.length>0){
            var queryCertificadosFlota = "Select c.idContratoCertificado, c.nroOrden, "+
                " c.nroCertificado as nCertificado, c.precio, c.valorCuota, c.nroCuota,"+
            ...
            ejecutarQUERY_MYSQL_Extra(resultados, queryCertificadosFlota, params, res, funcionName, function(res, resultados, resultadosPrevios){
                ...
                enviarResponse(res, resultadosPrevios)
            })
        }else{
            enviarResponse(res, [])
        }
