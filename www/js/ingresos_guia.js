	...
	try{
		if(validarCamposRequeridos("idPanel")){
			if($("#tabla_datos > tbody >tr").length>0){
				fancyConfirm("Continuar con la operacion", function(rpta){
					var parametrosPOST = {
						"tipo":"ING",
...
function editar(){
	try{
		if(filaSeleccionada!=undefined){
			...
			abrirVentanaFancyBox(730, 265, "nuevo_editar_detalle_guia?accion=E&tipo=ING&idDetalle="+idDetalle+"&idAlmacen="+idAlmacen, true, function(rptaDatos){
				...
				for(var i=0; i<arrayDatos.length; i++){
					if(arrayDatos[i].idDetalle == idDetalle){
						...
						arrayDatos[i].estado = rptaDatos[0].estado
						break;
					}				
				}
...
function nuevo(){
	try{
		...
		var idAlmacen = $("#idCmbAlmacen").val();
		if(idAlmacen!=""){			
			abrirVentanaFancyBox(730, 265, "nuevo_editar_detalle_guia?accion=N&tipo=ING&idDetalle="+contadorId+"&idAlmacen="+idAlmacen, true, function(rptaDatos){
