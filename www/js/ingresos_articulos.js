...
function listar(resultsData){ // crea la grilla con la paginacion
	try{
        ...
        var camposAmostrar = [ // asigna los campos a mostrar en la grilla
            ...
            {campo:'nombreProveedor', alineacion:'center'}            
        ];
...
function revisarGuia(){
	try{
		if(filaSeleccionada==undefined){
			...
			parent.abrirVentanaFancyBox(1050, 560, "ingresos_guia?accion=V&idGuia="+arrayDatos[filaSeleccionada].idGuia, true);
		}		
	}catch(err){
...
function nuevaGuia(){
	try{
		parent.abrirVentanaFancyBox(1050, 560, "ingresos_guia?accion=N", true, function(){
			buscar();
		}, true);				
	}catch(err){
