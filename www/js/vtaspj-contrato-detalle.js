             ...
             DAOV.consultarWebServiceGet("getAllDistritos", "", function(data) {
                 ...
                 DAOV.consultarWebServiceGet("getAllProvincias", "", function (datos) {
                     ...
                     DAOV.consultarWebServiceGet("getAllDepartamentos", "", function (depas) {
                         ...
                         if(accion=='N'){
                             $("#btnImprimirCAT").css("display","none"); // oculta el boton de impresion
                             $.fancybox.close();
                         }else{
                             ...
                             DAOV.consultarWebServiceGet("getContratoDetalle", parametros, function(registroContrato){
                                 ...
                                 if(detalleContrato[0].estado!='R'){
                                     $("#btnImprimirCAT").css("display","none");
 ...
 function actualizarTotales(){
     try{
         ...
         for(var i=0; i<arrayDatos.length; i++){
             ...
             totalPrimas += parseFloat(arrayDatos[i].prima);
         }
         $("#txtTotalPrecio").val(formatDec(totalPrecios,"S/.",1));
         $("#txtTotalAporte").val(formatDec((totalPrecios-totalPrimas),"S/.",1));
         $("#txtTotalFondo").val(formatDec(totalPrimas,"S/.",1)); // => TOTAL PRIMA
     }catch(err){
 ...
 */
                 ...
                 $("#tr_" + idDetalle).find("td").eq(9).html(rptaDatos[0].prima);
                 $("#tr_" + idDetalle).find("td").eq(10).html(rptaDatos[0].aporte);
...
//------------------------------------------------------------
     ...
     try{
         ...
         if(datosCompletos){
             ...
             $("#tabla_datos > tbody >tr").each(function(){
                 ...
                 var prima = $(this).find("td").eq(10).text()

                 if(!(asientos!="0" && precio!="0" && aporte!="0" && prima!="0")){
                     flotaCompleta=false
                     return false
                 }
 ...
 function impresionCATS(){
     try{
         fancyConfirm("¿Esta seguro de proceder con la operación?", function(rpta){
             if(rpta){
                 var fechaRenovacion = dateTimeFormat($("#txtVigCert_Inicio").val())
                 detalleContrato[0].fechaRenovacion = fechaRenovacion
                 detalleContrato[0].idConcesionario = idConcesionarioVtasCorp
                 detalleContrato[0].idUsuarioUpdate=parent.idUsuario
                 detalleContrato[0].fechaEmision=dateTimeFormat($("#txtFechaEmision").val())
                 var datosJSON = detalleContrato[0]
                 DAOV.consultarWebServicePOST(datosJSON, "imprimirCATS", function(data){
                     var filasAfectadas = data[0]
                     if(filasAfectadas>0){
                         fancyAlertFunction("¡Operación Exitosa!", function(rpta){
                             if(rpta){
                                 // genera el excel
                                 window.open("wbs_ventas?funcion=reporteContratoExcel&idContrato="+idContrato,'_blank');
                                 realizoTarea=true
                                 parent.$.fancybox.close();
                             }
                         })
         ...
         emitirErrorCatch(err, "impresionCATS()")
     }
 }
