...
var urldominio="http://autoseguro.xyz/";
...
exports.getEventosGenerales = function(req, res, funcionName){ // FALTCOMP
	...
	if(tipoBusqueda=="A"){
        ...
        query="SELECT e.codEvento, e.estado, e.condonado, e.nroCAT , c.placa, date_format (e.fechaAccidente, '%d/%m/%Y %H:%i:%s') as fechaAccidente ,"+
            ...
            "pc.nombres as nombresChofer, pc.apellidoPaterno as apellidoPaternoChofer, pc.apellidoMaterno as apellidoMaternoChofer, pc.nroDocumento as dniChofer, (concat(pc.calle,' ',pc.nro,' ',pc.mzLote,' ',pc.sector)) as direccionChofer, pc.telefonoFijo as telefonoFijoChofer, pc.telefonoMovil as celularChofer, dc.nombre as distritoChofer, "+ 
            "pa.idPersona as idPersonaAsociado, pa.nombres as nombresAsociado, pa.apellidoPaterno as apellidoPaternoAsociado, "+
            "pa.apellidoMaterno as apellidoMaternoAsociado, pa.tipoPersona as tipoAsociado, pa.razonSocial, (concat(pa.calle,' ',pa.nro,' ',pa.mzLote,' ',pa.sector)) as direccionAsociado, pa.telefonoFijo as telefonoFijoAsociado, pa.telefonoMovil as celularAsociado, da.nombre as distritoAsociado, c1.descripcion as causal1, c2.descripcion as causal2, ch.licenciaChofer "+    
...
exports.getGastosByCodEvento = function(req, res, funcionName){ // FALTA
    ...
    //actualizarGastos($codEvento, res, funcionName, function(res, resultados){
	    ...
	    var $query="select * from ((select concat('(Carta) ',g.nroCarta) as numero, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, "+
	       ...
	       " where g.codEvento='"+$codEvento+"'"+$queryAdicional1+" and g.estado='P') UNION ALL  (select concat('N° IPRESS ', g.nroDocumento) as numero, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, "+
	       " g.monto from OrdenPago g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto "+
	       " inner join Agraviado a on g.codAgraviado=a.codAgraviado "+
	       " inner join Persona p on a.idPersona=p.idPersona "+
	       " where g.codEvento='"+$codEvento+"'"+$queryAdicional2+" and g.estado in ( 'B', 'P' )) UNION ALL (select concat('N° IPRESS ', g.nroDocumento) as numero, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fechaDoc, '%d/%m/%Y') as fechaDoc, "+
	       " g.monto from Gasto g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto "+
	       " inner join Agraviado a on g.codAgraviado=a.codAgraviado "+
	       " inner join Persona p on a.idPersona=p.idPersona "+
	       " where g.codEvento='"+$codEvento+"'"+$queryAdicional+")) as v order by v.tipoGasto, STR_TO_DATE(v.fechaDoc, '%d/%m/%Y') ";
...
exports.buscarAcuerdosConOrdenesPago = function(req, res, funcionName){ // FALTCOMP
    ...
    var query="Select LPAD(A.idAcuerdo,5,'0') as idAcuerdo, A.codEvento, DATE_FORMAT (A.fechaAcuerdo, '%d/%m/%Y') as fechaAcuerdo, A.deudaAcordada, A.cuotaInicial, "+
...
exports.buscarAcuerdos = function(req, res, funcionName){ // FALTCOMP
    ...
    if(tipoBusqueda=="F"){ // fecha
  	...
  	var query="Select LPAD(A.idAcuerdo,5,'0') as idAcuerdo, A.codEvento, date_format( A.fechaAcuerdo, '%d/%m/%Y') as fechaAcuerdo, A.gastosAdministrativos, A.deudaAcordada, A.cuotaInicial, "+
        "date_format(A.fechaInicioCuotas, '%d/%m/%Y') as fechaInicioCuotas, date_format(E.fechaAccidente, '%d/%m/%Y') as fechaAccidente, TP.descripcion, concat(P.nombres,' ',P.apellidoPaterno,' ',P.apellidoMaterno) as nombreAsociado, "+
...
exports.liquidacionPDF=function(req, res, funcionName){
	...
	var $query="select * from ((select g.nroCarta as numero, if(g.idEtapa='5', f.nombre, n.nombre) as proveedor, 'CARTA GARANTIA' as comprobante, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, "+
	       " g.monto from CartaGarantia g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto "+
	       " inner join Agraviado a on g.codAgraviado=a.codAgraviado "+
	       " inner join Persona p on a.idPersona=p.idPersona "+
		   " left join Nosocomio n on g.idNosocomio = n.idNosocomio "+
		   " left join Funeraria f on g.idFuneraria = f.idFuneraria "+
	       " where g.codAgraviado='"+$numsiniestro+"' and g.estado='P') UNION ALL  (select g.nroDocumento as numero, if(ppro.tipoPersona='J', ppro.razonSocial, concat(ppro.nombres,' ',ppro.apellidoPaterno,' ', ppro.apellidoMaterno)) as proveedor, t.descripcion as comprobante, tg.descripcion as tipoGasto, concat(p.nombres,' ',p.apellidoPaterno,' ',p.apellidoMaterno) as nombresAgraviado, DATE_FORMAT (g.fecha, '%d/%m/%Y') as fechaDoc, "+
	       " g.monto from OrdenPago g inner join TipoGasto tg on g.idEtapa = tg.idTipoGasto "+
	       " inner join Agraviado a on g.codAgraviado=a.codAgraviado "+
	       " inner join Persona p on a.idPersona=p.idPersona "+
		   ...
		   " left join Persona ppro on prov.idPersona = ppro.idPersona "+
		   " left join TipoDocumento t on g.idTipoDocumento = t.idTipoDocumento "+
	       " where g.codAgraviado='"+$numsiniestro+"' )) as v order by v.tipoGasto, STR_TO_DATE(v.fechaDoc, '%d/%m/%Y')";
	    ...
	    ejecutarQUERY_MYSQL($query, [], res, funcionName, function(res, resultados){
	    	...
	    	for($cont; $cont<resultados.length; $cont++){
	    		if($cont==0){ // La primera vez
                	...
                	"<div style='font-size:14px; font-weight:bold; margin-left:14px;'>"+resultados[$cont]['tipoGasto']+"</div>"+$cabecera+
                    "<br>"+
                    "<tr>"+
	                ...
	                if(resultados[$cont]['tipoGasto']!=$descripcionAnterior){
	                    ...
	                    "</tr>"+
	                    "</table><br>"+
	                    "<div style='font-size:14px; font-weight:bold; margin-left:14px;'>"+resultados[$cont]['tipoGasto']+"</div><br>"+$cabecera;
	                    $totalTipo=0;
	                }
	                $tablasDetalle+="<tr>"+
	                   "<td>"+resultados[$cont]['numero']+"</td>"+
        	...
        	var body=
                ...
                "<body style='padding-top:0px; background-color:;'>"+
    				...
    				"<table style='width:635px; font-size:14px; margin:auto;'>"+
				        "<tr>"+
				            "<td style='text-align: left; padding-top:5; width: 315px; color:gray; font-weight:bold; background-color:;'>"+
				                '<img src="http://sistema.autoseguro.pe/wpimages/logo_autoseguro.jpg" style="width:140px;">'+
				                '<br><br>'+
				    ...
				    "<br>"+
				    "<br>"+
				    "<label style='font-weight:bold; font-size:14px;'>TOTAL DE GASTOS REALIZADOS A LA FECHA:  S/. "+$totalGlobal+"</label>"+
				    "<br>"+
				    "<label style='font-size:14px;'>*** Estos Gastos se pueden incrementar, conforme se vaya generando en su futuro, conforme a las normas legales ***</label>"+
				    "</div>"+
                "</body>"+
                "</html>";
...
exports.busquedaAvanzadaTramites = function(req, res, funcionName){ // Realiza la busqueda avanzada para los movimientos de tramites documentarios
	...
	var query = "Select h.idHistorial, LPAD(e.idExpediente,5,'0') idExpediente, e.tipoExpediente, h.idArea as idAreaOrigen, Aorigen.Nombre as nombreArea, "+ "h.idAreaDestino, date_format (h.fechaIngreso, '%Y-%m-%d %T') as fechaIngreso, "+
	...
	"date_format (event.fechaAccidente, '%d/%m/%Y') as fechaAccidente from Expediente e "+
