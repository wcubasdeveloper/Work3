...
function erroresXHR(jqXHR, textStatus, errorThrown) {
    ...
    if (jqXHR.status === 0) {
        ...
        msj='Erro Intetrno de Servidor Remoto [500].';
        ...
        $.post("webservice?funcion="+funcion_webService, formData,
            function(data){
                ...
                function(jqXHR, textStatus, errorThrown) {
                    var msj="";
                    if (jqXHR.status === 0) {
                        msj='Not connect: Verify Network.';
                    } else if (jqXHR.status == 404) {
                        msj='Requested page not found [404]';
                    } else if (jqXHR.status == 500) {
                        msj='Internal Server Error [500].';
                    } else if (textStatus === 'parsererror') {
                        msj='Requested JSON parse failed.';
                    } else if (textStatus === 'timeout') {
                        msj='Time out error.';
                    } else if (textStatus === 'abort') {
                        msj='Ajax request aborted.';
                    } else {
...
function agregarOpcionesToCombo(idSelect, datos, campos, isSelect2 = false, openSelect = false){ // Agrega los datos de un Array asociativo {id:"", texto:""} en un combobox
    try{
...
// Extension: funciones usadas en interfaces con 2 DataTables, cada uno maneja su juego de datos
//********* PARA UTLIZAR LA SELECCION DE FILAS ES NECESARIO CREAR LA VARIABLE GLOBAL filaSeleccionada1=undefined;
var filaSeleccionada1=undefined; // Fila que contiene el indice del elemento (del arreglo) que contiene el registro seleccionado
function borrarFilaSeleccionada1(){ // borra una fila seleccionada (valor de la variable y la despinta)
    try{
        if(filaSeleccionada1!=undefined){ // Identifica que se haya seleccionado un registro
            var TDs=$("#tr1_"+filaSeleccionada1).find("td"); // Busca todos los <TD></TD> dentro de la Fila <TR></TR>
            TDs.each(function(){ // agrega estilo a cada <TD></TD>
                $(this).css("background-color", "transparent"); // Lo vuelve a color transparente
                $(this).css("color", colorFuenteAntesDePintar); // Asigna como color negro a la fuente.
            });
            filaSeleccionada1=undefined; // borra la informacion de la fila que fue seleccionada
        }
    }catch(err){
        emitirErrorCatch(err, "borrarFilaSeleccionada1"); // emite error
    }
}
function pintarNuevaFilaSeleccionada1(id){ // Pinta una nueva fila y agregar su valor a la variable in
    try{
        var TDs=$("#tr1_"+id).find("td"); // Busca todos los TD dentro de la Fila
        TDs.each(function(){ // Pinta cada td encontrado
            colorFuenteAntesDePintar=$(this).css("color");
            $(this).css("background-color", "gray");
            $(this).css("color", "white");
        });
        filaSeleccionada1=id;
    }catch(err){
        emitirErrorCatch(err, "pintarFilaSeleccionada1"); // emite error
    }
}
function seleccionarFila1(id){ // Selecciona una fila (despinta la anterior y selecciona la nueva fila)
    try{
        if(id!=filaSeleccionada1){
            borrarFilaSeleccionada1(); // Borra la fila seleccionada anteriormente, si es que lo hubiera
            pintarNuevaFilaSeleccionada1(id); // pinta y asigna nueva fila seleccionada
        }
    }catch (err){
        emitirErrorCatch(err, "seleccionarFila1"); // emite error
    }
}
function crearFilasHTML1(idTablaHTML, datos, campoAlineacionArray, ONCLIK_FILA_SELECCIONADA, fontSize, idPrefijo){
    try{
        ...
        if(datos.length>0){
            ...
            for(var i=0; i<datos.length; i++){
                if(ONCLIK_FILA_SELECCIONADA){ // si es TRUE
                    onclick="onclick='seleccionarFila1("+'"'+idPrefijo+i+'"'+")' id='tr_"+idPrefijo+i+"'"; //***Para el 2do DataTable***
                }
                filaTRAppend+="<tr  style='font-family: Arial; height: 30px; "+cursor+" font-size: "+fontSize+"px;' "+onclick+">";
                for(var y=0; y<cantidadAtributos; y++){ //completa las columnas segun la cantidad de atributos
                    AlineacionTD="justify";
...
function parseDataTable1(idTabla, columnWidthArray, scrollY, orderByColumn, ajustableAlto, searching, paging, functionInit){
    try{
        borrarFilaSeleccionada1();
        var sort=true;
        if(orderByColumn==undefined || orderByColumn==false){
            sort=false;
            orderByColumn=[0, "desc"];
        ...
        emitirErrorCatch(err, "parseDataTable1")
    }
}

//Formatea Numero decimal con simbolo monetario y separado por miles
function formatDec(numero, simbolo, nDec) {
    return simbolo + " " + numero.toFixed(nDec).replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
}

function number_format(number, decimals, dec_point, thousands_sep) { // formatea un numero entero o decimal colocando separacion por coma o punto
    number = (number + '')
        .replace(/[^0-9+\-Ee.]/g, '');
...
function campoFecha(id, conHora){
    try{
        var format = 'd/m/Y'
        if(conHora==true){
            format = format+' H:i'
        ...
        $("#"+id).datetimepicker({lan:'es', format:format,  timepicker:conHora, closeOnDateSelect:!conHora, step:15});
    }catch(err){
        emitirErrorCatch(err, "campoFecha")
    }
}
// funcion generica para limpiar fechas
...
function asignarNumericos(){
	try{
		$(".solo-numero").keypress(function(e){ // permite ingresar solo numeros
			return textNumber(e);
		});
	}catch(err){
		emitirErrorCatch(err, "asignarNumericos")
	}
}
function asignarDecimalNumericos(){
	$(".decimales").keypress(function(e){
		return validateFloatKeyPress(this, e);
	});
}

// funcion para generar decimales con dos decimas
