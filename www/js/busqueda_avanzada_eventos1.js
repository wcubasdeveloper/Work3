	...
	$("input[type='radio'][name='busqueda']").change(
		function(){
			...
			if(valorSeleccionado=='F'){
                ...
                if(valorSeleccionado=="G"){
                    nombreLabel="Cod. Agraviado :";
                }
				$("#buscar_x_Agraviado").css("display", "none");
				$("#buscar_x_fechas").css("display", "none");
				$("#buscar_x_codigo").css("display", "block");
				$("#wb_idLabel").html("<span class='Body-C'>"+nombreLabel+"</span>");
				$("#codigo").focus();
			}
		}
	);
...
function BuscarEvento1(){
	try{
		if($("input:checked").length>0){
            ...
            if(valorSeleccionado=="A" ){ // Agraviado
				if($("#nombreAgraviado").val().trim()=="" && $("#apellidoAgraviado").val().trim()==""
                    && $("#dniAgraviado").val().trim()=="" && $("#codigo").val().trim()==""){
					fancyAlertFunction("Debe ingresar al menos un dato del agraviado", function(rpta){
						if(rpta){$("#nombreAgraviado").focus();	}
					    });
					return;
				}else {
            ...
            }else if( valorSeleccionado=="G"){ // Cod Agraviado
                if($("#codigo").val().trim()==""){
                    fancyAlertFunction("Debe ingresar el codigo del Agraviado!", function(rpta){
                        if(rpta){ $("#codigo").focus(); }
                    });
                    return;
                }else{
                    //parametros+="&nombreAgraviado=''&apellidoAgraviado=''&dniAgraviado=''&codAgraviado="+$("#codigo").val();
                    parametros+="&codAgraviado="+$("#codigo").val();
                }
			}else {
                if(valorSeleccionado=="F")
                {// si se selecciono por Fecha se debe validar que la fecha de inicio se haya completado
                    if (($("#inicio").val().trim()=="" && $("#fin").val().trim()=="") || ($("#inicio").val() == $("#fin").val() )) {
                        fancyAlertFunction("Debe ingresar la fecha de Inicio y/o Fin y no deben ser iguales", function (estado) {
                            if (estado) { $("#inicio").focus(); }
                        });
                        return;
                    } else {
                        campoValidar="inicio-Fecha de Inicio";
                        parametros+="&fechaInicio="+dateTimeFormat($("#inicio").val())+ "&fechaFin="+dateTimeFormat($("#fin").val());
                    }
                } else
                {
		...
		emitirErrorCatch(err, "BuscarEvento1"); // emite error
	}
}
		...
		if(filaSeleccionadaEvento!=undefined){
			...
			//	if(estado){
					...
					rptaCallback=[{
                        ...
                        descripcionEvento:arrayDelEvento.descripcionEvento,
