...
cargarInicio(function(){
	...
	$("#idObservaciones").attr("maxlength", "1000"); // asigna 1000 caracteres para las observaciones del informe	
	if(accion=='N'){
		$("#idBtnPDF").css("display", "none");
	}
	...
	DAO.consultarWebServiceGet("consultarCostosGlobales", "", function(datos){
		...
		DAO.consultarWebServiceGet("getTipoAtencionList", "", function(tipoAtencion){
			// solo obtiene los tipos de atencion: Emergencia, Urgencia e Internamiento			
			//listaTipoAsitencia = tipoAtencion;
...
function crearComboTipoAtencion(data, tipoAtencion){
	try{
		var stringCombo = "<select style='width:130px;' id='tipoAsistencia_"+data.codAgraviado+"' onchange='calcularMontoGarantia("+'"'+data.codAgraviado+'"'+")'>";
		if(accion=='N'){ // Nuevo Informe
			stringCombo = stringCombo+"<option value=''>Seleccione</option>";
		}
		for(var i=0; i<listaTipoAsitencia.length; i++){
			var selected = "";
			if(listaTipoAsitencia[i].idTipoAtencion==tipoAtencion){
				selected = "selected";
			}			
			stringCombo = stringCombo+"<option value='"+listaTipoAsitencia[i].idTipoAtencion+"' "+selected+">"+listaTipoAsitencia[i].descripcion+"</option>";
		}
        stringCombo = stringCombo+"</select>";
		return stringCombo;
	}catch(err){
		emitirErrorCatch(err, "crearComboTipoAtencion");
	}
}
        ...
        switch(accion){
            case 'N':
                for(var i=0; i<data.length; i++){
                    ...
                    data[i].html_asistencia = crearComboTipoAtencion(data[i], data[i].tipoAsistencia);
...
function agregarAgraviado(){
    try{
		...
		var objectAgraviado = {codAgraviado:codAgraviado};
        var trAgraviado="<tr style='font-family: Arial; height: 30px; cursor: pointer; font-size: 12px;'>" +
            ...
            "<td style='text-align: center;'>"+crearComboTipoAtencion(objectAgraviado)+"</td>"+
...
function buscarPersona(tipoPersona){
	try{
		...
		if(cantidadDigitos == 8){
			if(DNI=='99999999'){ // No identificado
				cargarResultPersona([], tipoPersona); 
				$("#idNombres_"+tipoPersona).val("----------");
				$("#idApePat_"+tipoPersona).val("----------");
				$("#idApeMat_"+tipoPersona).val("----------");
				$("#idNombres_"+tipoPersona).blur();
			}else{
            ...
            if(validarCamposRequeridos("idPanelDireccionAsociado")){
                if($("#nroCAT").prop("disabled")==true){ // se valido el CAT correctamente
                    ...
                    if($("#idVencPoliza").val()!=""){
                        ...
                        var DateVencimiento = new Date(fechaVencimiento[2], (parseInt(fechaVencimiento[1])-1), fechaVencimiento[0]);
						DateVencimiento.setDate(DateVencimiento.getDate()+1);
                        var fechaAccidenteDate = $("#idFechaAccidente").val();
						if(fechaAccidenteDate==""){
							fancyAlertFunction("¡ Debe ingresar la fecha del accidente !", function(){
								$("#idFechaAccidente").focus();
							});
							continuous=false;
							return;
						}
						fechaAccidenteDate = fechaAccidenteDate.split(" ");
						var soloFecha = fechaAccidenteDate[0].split("/");
						var soloHora = fechaAccidenteDate[1].split(":");
						
						var dateAccidente = new Date(soloFecha[2], (parseInt(soloFecha[1])-1), soloFecha[0], soloHora[0], soloHora[1], 0, 0);
						if(dateAccidente>DateVencimiento){
                            continuous=false;
                            fancyAlert("¡ No se puede registrar el Informe, porque el CAT Caducó antes que ocurriera el Accidente !");
                        }
                    }
                    if(continuous){
