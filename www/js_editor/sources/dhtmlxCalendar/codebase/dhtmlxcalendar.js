...
function dhtmlXCalendarObject(inps, skin) {
	...
	li.innerHTML = "<div class='dhtmlxcalendar_time_img'></div>"+
			...
			"<span class='dhtmlxcalendar_label_clear'>Clear</span><span class='dhtmlxcalendar_label_today'>Today</span>"; // added in 4.6
	ul.appendChild(li);
	...
	li.onclick = function(e) {
		...
		// hide selector if it visible
		that._hideSelector();
		// today/cleatr buttons, added in 4.6
		if (t.className && t.className == "dhtmlxcalendar_label_today") {
			var d = new Date();
			d.setHours(that._activeDate.getHours());
			d.setMinutes(that._activeDate.getMinutes());
			d.setSeconds(that._activeDate.getSeconds());
			that.setDate(d);
		}
